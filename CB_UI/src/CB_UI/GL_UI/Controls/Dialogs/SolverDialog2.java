...
public class SolverDialog2 extends ActivityBase implements OnStateChangeListener
{
	...
	private Solver solver; // Solver Object dieses Caches f�r die Functions, Variablen...
	...
	private boolean isFunction(String string)
	{
		...
		// in eine g�ltigen Formel d�rfen nur normale Buchstaben oder Zahlen stehen
		for (int i = 0; i < posKlammerAuf; i++)
		{
			char c = formula.charAt(i);
			if (!Character.isLetter(c) && !Character.isDigit(c)) return false;
		}
		// g�ltige Formel erkannt anhand dem Format.
		return true;
	}
