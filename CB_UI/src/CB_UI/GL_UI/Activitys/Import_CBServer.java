...
public class Import_CBServer extends ActivityBase implements ProgressChangedEvent {
	...
	for (ExportEntry entry : exportList) {
	    ...
	    case NotesText:
		// Note Text laden
		entry.note = Database.Data.GetNote(entry.cacheId);
		break;
	    case SolverText:
		// Solver Text laden
		entry.solver = Database.Data.GetSolver(entry.cacheId);
		break;
	    default:
		break;
	    }
	...
	CoreCursor reader = Database.Data.db.rawQuery(WaypointDAO.SQL_WP_FULL + " where GcCode = ?", new String[] { wpGcCode });
	reader.moveToFirst();
