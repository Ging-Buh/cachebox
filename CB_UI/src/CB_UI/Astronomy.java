...
public class Astronomy {
	...
	public static double UtcToJulianDate(Date date) {
		synchronized (calender) {
			...
			return Math.floor(365.25 * jahr) + Math.floor(30.6001 * (monat + 1.0)) - 15 + 1720996.5 + day + (double) hour / 24.0 + (double) minute / 1440 + (double) second / 86400.0;
		}
	}
	...
	public static Coordinate EclipticToEquatorial(Coordinate eclipticCoordinate, double julianDate) {
		...
		double lat = (MathUtils.RAD_DEG * Math.atan2((sinlon * coseps - Math.tan(eclipticCoordinate.getLatitude() / 180.0 * Math.PI) * sineps), Math.cos(eclipticCoordinate.getLongitude() / 180.0 * Math.PI))) % 360; // rektaszension (alpha)

		double lon = 180.0 / Math.PI * Math.asin(Math.sin(eclipticCoordinate.getLatitude() / 180.0 * Math.PI) * coseps + Math.cos(eclipticCoordinate.getLatitude() / 180.0 * Math.PI) * sineps * sinlon); // deklination (delta)
	...
	public static Coordinate EquatorialToAzymuth(Coordinate observerPosition, double julianDate, Coordinate equatorialPosition) {
		...
		double azimutNenner = Math.cos(tau) * Math.sin(phi) - Math.tan(equatorialPosition.getLatitude() / 180.0 * Math.PI) * Math.cos(observerPosition.getLatitude() / 180.0 * Math.PI);
		...
		if (azimutNenner < 0)
			azimut += 180.0f;
		double h = MathUtils.RAD_DEG * Math.asin(Math.cos(equatorialPosition.getLatitude() / 180.0 * Math.PI) * Math.cos(tau) * Math.cos(phi) + Math.sin(equatorialPosition.getLatitude() / 180.0 * Math.PI) * Math.sin(phi));
	...
	public static Coordinate EclipticCoordinatesMoon(double julianDate) {
		...
		double sunAnomalyMean = 360.0 * MathUtils.DEG_RAD / 365.242191 * (julianDate - 2447891.5) + 279.403303 * MathUtils.DEG_RAD - 282.768422 * MathUtils.DEG_RAD;
