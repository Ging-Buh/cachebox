...
{
	public static final int CurrentRevision = 2352;
	...
	public static boolean APIisOnline()
	{
		...
		{
			Gdx.app.log(Tag.TAG, "GlobalCore.APIisOnline() - no GC - API AccessToken");
			return false;
		}
	...
	public static boolean JokerisOnline()
	{
		...
		if (JokerPwExist)
		{
			// Gdx.app.log(Tag.TAG,"GlobalCore.JokerisOnline() - no Joker Password");
			return false;
		}
		if (platformConector.isOnline())
		{
	...
	public static void checkSelectedCacheValid()
	{
		...
		if ((List.size() > 0) && (GlobalCore.ifCacheSelected()) && (List.GetCacheById(GlobalCore.getSelectedCache().Id) == null))
		{
			...
			Gdx.app.debug(Tag.TAG, "Change SelectedCache from " + GlobalCore.getSelectedCache().getGcCode() + "to" + List.get(0).getGcCode());
		...
		{
			GlobalCore.setSelectedCache(List.get(0));
			Gdx.app.debug(Tag.TAG, "Set SelectedCache to " + List.get(0).getGcCode() + " first in List.");
		}
	}
