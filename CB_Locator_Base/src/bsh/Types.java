...
class Types {
	...
	/**
	 * Special value that indicates by identity that the result of a cast operation was a valid cast. This is used by castObject() and
	 * castPrimitive() in the checkOnly mode of operation. This value is a Primitive type so that it can be returned by castPrimitive.
	 */
	...
	/**
	 * Is the 'from' signature (argument types) assignable to the 'to' signature (candidate method types) This method handles the special
	 * case of null values in 'to' types indicating a loose type and matching anything.
	 */
	...
	/**
	 * Test if a conversion of the rhsType type to the lhsType type is legal via standard Java assignment conversion rules (i.e. without a
	 * cast). The rules include Java 5 autoboxing/unboxing.
	 * <p/>
	 * For Java primitive TYPE classes this method takes primitive promotion into account. The ordinary Class.isAssignableFrom() does not
	 * take primitive promotion conversions into account. Note that Java allows additional assignments without a cast in combination with
	 * variable declarations and array allocations. Those are handled elsewhere (maybe should be here with a flag?)
	 * <p/>
	 * This class accepts a null rhsType type indicating that the rhsType was the value Primitive.NULL and allows it to be assigned to any
	 * reference lhsType type (non primitive).
	 * <p/>
	 * Note that the getAssignableForm() method is the primary bsh method for checking assignability. It adds additional bsh conversions,
	 * etc.
	 * 
	...
	/**
	 * Attempt to cast an object instance to a new type if possible via BeanShell extended syntax rules. These rules are always a superset
	 ...
	 * <p/>
	 * This method can handle fromValue Primitive types (representing primitive casts) as well as fromValue object casts requiring interface
	 * generation, etc.
	 * 
	 * @param toType
	 *            the class type of the cast result, which may include primitive types, e.g. Byte.TYPE
	 * @param fromValue
	 *            an Object or bsh.Primitive primitive value (including Primitive.NULL or Primitive.VOID )
	 * @see #isBshAssignable(Class, Class )
	 */
	@SuppressWarnings("rawtypes")
	...
	/**
	 * Perform a type conversion or test if a type conversion is possible with respect to BeanShell extended rules. These rules are always a
	 * superset of the Java language rules, so this method can also perform (but not test) any Java language assignment or cast conversion.
	 * <p/>
	 * This method can perform the functionality of testing if an assignment or cast is ultimately possible (with respect to BeanShell) as
	 * well as the functionality of performing the necessary conversion of a value based on the specified target type. This combined
	 * functionality is done for expediency and could be separated later.
	 * <p/>
	 * Other methods such as isJavaAssignable() should be used to determine the suitability of an assignment in a fine grained or
	 * restrictive way based on context before calling this method
	 * <p/>
	 * A CAST is stronger than an ASSIGNMENT operation in that it will attempt to perform primtive operations that cast to a smaller type.
	 * e.g. (byte)myLong; These are used in explicit primitive casts, primitive delclarations and array declarations. I don't believe there
	 * are any object conversions which are different between ASSIGNMENT and CAST (e.g. scripted object to interface proxy in bsh is done on
	 * assignment as well as cast).
	 * <p/>
	 * This method does not obey strictJava(), you must test first before using this method if you care. (See #isJavaAssignable()).
	 * <p/>
	 * 
	 * @param toType
	 *            the class type of the cast result, which may include primitive types, e.g. Byte.TYPE. toType may be null to indicate a
	 *            loose type assignment (which matches any fromType).
	 * @param fromType
	 *            is the class type of the value to be cast including java primitive TYPE classes for primitives. If fromValue is (or would
	 *            be) Primitive.NULL then fromType should be null.
	 * @param fromValue
	 *            an Object or bsh.Primitive primitive value (including Primitive.NULL or Primitive.VOID )
	 * @param checkOnly
	 *            If checkOnly is true then fromValue must be null. FromType is checked for the cast to toType... If checkOnly is false then
	 *            fromValue must be non-null (Primitive.NULL is ok) and the actual cast is performed.
	 * @throws UtilEvalError
	 *             on invalid assignment (when operation is assignment ).
	 * @throws UtilTargetError
	 *             wrapping ClassCastException on cast error (when operation is cast)
	 * @param operation
	 *            is Types.CAST or Types.ASSIGNMENT
	...
	/**
	 * Return a UtilEvalError or UtilTargetError wrapping a ClassCastException describing an illegal assignment or illegal cast,
	 * respectively.
	 */
	@SuppressWarnings("rawtypes")
