...
class BSHBinaryExpression extends SimpleNode implements ParserConstants
{
	...
	public Object eval(CallStack callstack, Interpreter interpreter) throws EvalError
	{
		...
		if (kind == INSTANCEOF)
		{
			 ...
			 */
			/*
			 * Primitive (number or void) is not normally an instanceof anything. But for internal use we'll test true for the bsh.Primitive
			 * class. i.e. (5 instanceof bsh.Primitive) will be true
			 */
			if (lhs instanceof Primitive) if (rhs == bsh.Primitive.class) return new Primitive(true);
			else
		...
		if (kind == BOOL_AND || kind == BOOL_ANDX)
		{
			...
			if (obj instanceof Boolean && (((Boolean) obj).booleanValue() == false)) return new Primitive(false);
		}
		/*
		 * Look ahead and short circuit evaluation of the rhs if: we're a boolean AND and the lhs is false.
		 */
		if (kind == BOOL_OR || kind == BOOL_ORX)
		{
			Object obj = lhs;
			if (isPrimitiveValue(lhs)) obj = ((Primitive) lhs).getValue();
			if (obj instanceof Boolean && (((Boolean) obj).booleanValue() == true)) return new Primitive(true);
		}
		...
		/*
		 * Doing the following makes it hard to use untyped vars... e.g. if ( arg == null ) ...what if arg is a primitive? The answer is
		 * that we should test only if the var is typed...? need to get that info here...
		 * 
		 * else { // Do we have a mixture of primitive values and non-primitives ? // (primitiveValue = not null, not void)
		 * 
		 * int primCount = 0; if ( isPrimitiveValue( lhs ) ) ++primCount; if ( isPrimitiveValue( rhs ) ) ++primCount;
		 * 
		 * if ( primCount > 1 ) // both primitive types, should have been handled above throw new InterpreterError("should not be here");
		 * else if ( primCount == 1 ) // mixture of one and the other throw new EvalError("Operator: '" + tokenImage[kind]
		...
		default:
			if (lhs instanceof Primitive || rhs instanceof Primitive) if (lhs == Primitive.VOID || rhs == Primitive.VOID) throw new EvalError(
					"illegal use of undefined variable, class, or 'void' literal", this, callstack);
			else if (lhs == Primitive.NULL || rhs == Primitive.NULL) throw new EvalError("illegal use of null value or 'null' literal",
					this, callstack);
