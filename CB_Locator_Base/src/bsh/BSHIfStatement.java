...
class BSHIfStatement extends SimpleNode
{
	...
	public Object eval(CallStack callstack, Interpreter interpreter) throws EvalError
	{
		...
		if (evaluateCondition((SimpleNode) jjtGetChild(0), callstack, interpreter)) ret = ((SimpleNode) jjtGetChild(1)).eval(callstack,
				interpreter);
		else if (jjtGetNumChildren() > 2) ret = ((SimpleNode) jjtGetChild(2)).eval(callstack, interpreter);
	...
	public static boolean evaluateCondition(SimpleNode condExp, CallStack callstack, Interpreter interpreter) throws EvalError
	{
		...
		{
			if (obj == Primitive.VOID) throw new EvalError("Condition evaluates to void type", condExp, callstack);
