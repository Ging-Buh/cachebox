 ...
 * </blockquote>
 * <p>
 * In the above examples we showed a single interpreter instance, however you may wish to use many instances, depending on the application
 * and how you structure your scripts. Interpreter instances are very light weight to create, however if you are going to execute the same
 * script repeatedly and require maximum performance you should consider scripting the code as a method and invoking the scripted method
 * each time on the same interpreter instance (using eval()).
 * <p>
...
public class Interpreter implements Runnable, ConsoleInterface, Serializable {
    ...
    /*
     * Debug utils are static so that they are reachable by code that doesn't necessarily have an interpreter reference (e.g. tracing in
     * utils). In the future we may want to allow debug/trace to be turned on on a per interpreter basis, in which case we'll need to use
     * the parent reference in some way to determine the scope of the command that turns it on or off.
     */
    ...
    /**
     * Compatibility mode. When {@code true} missing classes are tried to create from corresponding java source files. Default value is
     * {@code false}, could be changed to {@code true} by setting the system property "bsh.compatibility" to "true".
     */
     ...
     *
     * @param namespace      If namespace is non-null then this interpreter's root namespace will be set to the one provided. If it is null a new one
     *                       will be created for it.
     * @param parent         The parent interpreter if this interpreter is a child of another. May be null. Children share a BshClassManager with their
     *                       parent instance.
     * @param sourceFileInfo An informative string holding the filename or other description of the source from which this interpreter is reading...
     ...
     * <p>
     * Note: This is here for completeness. If you're using this a lot it may be an indication that you are doing more work than you have
     * to. For example, caching the interpreter instance rather than the namespace should not add a significant overhead. No state other
    ...
    /**
     * Spawn a non-interactive local interpreter to evaluate text in the specified namespace. Return value is the evaluated object (or
     * corresponding primitive wrapper).
     *
     * @param sourceFileInfo is for information purposes only. It is used to display error messages (and in the future may be made available to the
     *                       script).
     ...
     */
    /*
     * Can't this be combined with run() ? run seems to have stuff in it for interactive vs. non-interactive... compare them side by side
     * and see what they do differently, aside from the exception handling.
     */
    ...
    /**
     * Print an error message in a standard format on the output stream associated with this interpreter. On the GUI console this will
     * appear in red, etc.
     */
    @Override
    ...
    /**
     * Get a reference to the interpreter (global namespace), cast to the specified interface type. Assuming the appropriate methods of the
     * interface are defined in the interpreter, then you may use this interface from Java, just like any other Java object.
     * <p>
     * For example:
     *
     * <pre>
     ...
     * </pre>
     * <p>
     * Note that the interpreter does *not* require that any or all of the methods of the interface be defined at the time the interface is
     * generated. However if you attempt to invoke one that is not defined you will get a runtime exception.
     * <p>
     ...
     * </pre>
     * <p>
     * <em>Version requirement</em> Although standard Java interface types are always available, to be used with arbitrary interfaces this
    ...
    /**
     * Set an external class loader to be used as the base classloader for BeanShell. The base classloader is used for all classloading
     * unless/until the addClasspath()/setClasspath()/reloadClasses() commands are called to modify the interpreter's classpath. At that
     * time the new paths /updated paths are added on top of the base classloader.
     * <p>
     * BeanShell will use this at the same point it would otherwise use the plain Class.forName(). i.e. if no explicit classpath management
     * is done from the script (addClassPath(), setClassPath(), reloadClasses()) then BeanShell will only use the supplied classloader. If
     * additional classpath management is done then BeanShell will perform that in addition to the supplied external classloader. However
     * BeanShell is not currently able to reload classes supplied through the external classloader.
     * <p>
     *
    ...
    /**
     * Set strict Java mode on or off. This mode attempts to make BeanShell syntax behave as Java syntax, eliminating conveniences like
     * loose variables, etc. When enabled, variables are required to be declared or initialized before use and method arguments are reqired
     * to have types.
     * <p>
    ...
    /**
     * Specify the source of the text from which this interpreter is reading. Note: there is a difference between what file the interrpeter
     * is sourcing and from what file a method was originally parsed. One file may call a method sourced from another file. See SimpleNode
    ...
    /**
     * Get the parent Interpreter of this interpreter, if any. Currently this relationship implies the following: 1) Parent and child share
    ...
    /**
     * Specify whether, in interactive mode, the interpreter exits Java upon end of input. If true, when in interactive mode the interpreter
     * will issue a System.exit(0) upon eof. If false the interpreter no System.exit() will be done.
     * <p/>
    ...
    /**
     * Compatibility mode. When {@code true} missing classes are tried to create from corresponding java source files. The Default value is
     * {@code false}. This could be changed to {@code true} by setting the system property "bsh.compatibility" to "true".
     *
    ...
    /**
     * Setting compatibility mode. When {@code true} missing classes are tried to create from corresponding java source files. The Default
     * value is {@code false}. This could be changed to {@code true} by setting the system property "bsh.compatibility" to "true".
     *
