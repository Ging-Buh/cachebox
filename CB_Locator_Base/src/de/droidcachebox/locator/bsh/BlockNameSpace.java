...
/**
 * A specialized namespace for Blocks (e.g. the body of a "for" statement). The Block acts like a child namespace but only for typed
 * variables declared within it (block local scope) or untyped variables explicitly set in it via setBlockVariable(). Otherwise variable
 * assignment (including untyped variable usage) acts like it is part of the containing block.
 * <p>
 */
/*
 * Note: This class essentially just delegates most of its methods to its parent. The setVariable() indirection is very small. We could
...
class BlockNameSpace extends NameSpace {
    ...
    /**
     * Override the standard namespace behavior to make assignments happen in our parent (enclosing) namespace, unless the variable has
     * already been assigned here via a typed declaration or through the special setBlockVariable() (used for untyped args in try/catch).
     * <p>
     * i.e. only allow typed var declaration to happen in this namespace. Typed vars are handled in the ordinary way local scope. All
     * untyped assignments are delegated to the enclosing context.
     */
    /*
     * Note: it may see like with the new 1.3 scoping this test could be removed, but it cannot. When recurse is false we still need to set
     ...
     * <p/>
     * Normally a 'this' reference to a BlockNameSpace (e.g. if () { } ) resolves to the parent namespace (e.g. the namespace containing the
     * "if" statement). However when code inside the BlockNameSpace needs to resolve things relative to 'this' we must use the actual
     * block's 'this' reference. Name.java is smart enough to handle this using getBlockThis().
     *
     * @see #getThis(Interpreter) This getBlockThis( Interpreter declaringInterpreter ) { return super.getThis( declaringInterpreter ); }
     */
    ...
    /**
     * This method recurses to find the nearest non-BlockNameSpace parent. public NameSpace getParent() { NameSpace parent =
    ...
    /**
     * Get a 'this' reference is our parent's 'this' for the object closure. e.g. Normally a 'this' reference to a BlockNameSpace (e.g. if
     * () { } ) resolves to the parent namespace (e.g. the namespace containing the "if" statement).
     *
