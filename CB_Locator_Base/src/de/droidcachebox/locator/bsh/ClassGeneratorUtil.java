...
/**
 * ClassGeneratorUtil utilizes the ASM (www.objectweb.org) bytecode generator by Eric Bruneton in order to generate class "stubs" for
 * BeanShell at runtime.
 * <p/>
 * <p/>
 * Stub classes contain all of the fields of a BeanShell scripted class as well as two "callback" references to BeanShell namespaces: one
 * for static methods and one for instance methods. Methods of the class are delegators which invoke corresponding methods on either the
 * static or instance bsh object and then unpack and return the results. The static namespace utilizes a static import to delegate variable
 * access to the class' static fields. The instance namespace utilizes a dynamic import (i.e. mixin) to delegate variable access to the
 ...
 * initInstance() invokes the instance intializer code (init vars and instance blocks) and then delegates to the corresponding scripted
 * constructor method in the instance namespace. Constructors contain special switch logic which allows the BeanShell to control the calling
 * of alternate constructors (this() or super() references) at runtime.
 * <p/>
 * <p/>
 * Specially named superclass delegator methods are also generated in order to allow BeanShell to access overridden methods of the
 * superclass (which reflection does not normally allow).
 * <p/>
 *
 * @author Pat Niemeyer
 */
 ...
 */
public class ClassGeneratorUtil implements org.objectweb.asm.Constants {
    ...
    public ClassGeneratorUtil(Modifiers classModifiers, String className, String packageName, Class superClass, Class[] interfaces, Variable[] vars, DelayedEvalBshMethod[] bshmethods, NameSpace classStaticNameSpace, boolean isInterface) {
        ...
        this.superClassName = org.objectweb.asm.Type.getInternalName(superClass);
        if (interfaces == null) {
    ...
    /**
     * Translate de.droidcachebox.locator.bsh.Modifiers into ASM modifier bitflags.
     */
     ...
     */
    private static void generateField(String fieldName, String type, int modifiers, org.objectweb.asm.ClassWriter cw) {
        cw.visitField(modifiers, fieldName, type, null/* value */);
    }

    /**
     * Generate a delegate method - static or instance. The generated code packs the method arguments into an object array (wrapping
     * primitive types in de.droidcachebox.locator.bsh.Primitive), invokes the static or instance namespace invokeMethod() method, and then unwraps / returns the
     * result.
     */
    private static void generateMethod(String className, String fqClassName, String methodName, String returnType, String[] paramTypes, int modifiers, org.objectweb.asm.ClassWriter cw) {
        ...
        org.objectweb.asm.CodeVisitor cv = cw.visitMethod(modifiers, methodName, methodDescriptor, exceptions);
        ...
        cv.visitMethodInsn(INVOKEVIRTUAL, "bsh/This", "invokeMethod", org.objectweb.asm.Type.getMethodDescriptor(org.objectweb.asm.Type.getType(Object.class),
                new org.objectweb.asm.Type[]{org.objectweb.asm.Type.getType(String.class), org.objectweb.asm.Type.getType(Object[].class), org.objectweb.asm.Type.getType(Interpreter.class), org.objectweb.asm.Type.getType(CallStack.class), org.objectweb.asm.Type.getType(SimpleNode.class), org.objectweb.asm.Type.getType(Boolean.TYPE)}));
    ...
    private static void doSwitchBranch(int index, String targetClassName, String[] paramTypes, org.objectweb.asm.Label endLabel, org.objectweb.asm.Label[] labels, int consArgsVar, org.objectweb.asm.CodeVisitor cv) {
    ...
    /*
     * Generate a branch of the constructor switch. This method is called by generateConstructorSwitch. The code generated by this method
     * assumes that the argument array is on the stack.
     */
    ...
    /**
     * Generate a superclass method delegate accessor method. These methods are specially named methods which allow access to overridden
     * methods of the superclass (which the Java reflection API normally does not allow).
     */
    // Maybe combine this with generateMethod()
    private static void generateSuperDelegateMethod(String superClassName, String methodName, String returnType, String[] paramTypes, int modifiers, org.objectweb.asm.ClassWriter cw) {
        String[] exceptions = null;
        ...
        // Add method body
        org.objectweb.asm.CodeVisitor cv = cw.visitMethod(modifiers, "_bshSuper" + methodName, methodDescriptor, exceptions);
     ...
     */
    private static void generatePlainReturnCode(String returnType, org.objectweb.asm.CodeVisitor cv) {
    ...
    /**
     * Generates the code to reify the arguments of the given method. For a method "int m (int i, String s)", this code is the bytecode
     * corresponding to the "new Object[] { new de.droidcachebox.locator.bsh.Primitive(i), s }" expression.
     *
     ...
     * @author Pat Niemeyer
     */
    private static void generateParameterReifierCode(String[] paramTypes, boolean isStatic, final org.objectweb.asm.CodeVisitor cv) {
        ...
        for (int i = 0; i < paramTypes.length; ++i) {
            ...
            if (isPrimitive(param)) {
                ...
                // as de.droidcachebox.locator.bsh.Primitive.NULL. However the This.invokeMethod()
    ...
    /**
     * Generates the code to unreify the result of the given method. For a method "int m (int i, String s)", this code is the bytecode
     ...
     * @author Pat Niemeyer
     */
    private static void generateReturnCode(String returnType, org.objectweb.asm.CodeVisitor cv) {
        if (returnType.equals("V")) {
            cv.visitInsn(POP);
            cv.visitInsn(RETURN);
        } else if (isPrimitive(returnType)) {
    ...
    /**
     * Evaluate the arguments (if any) for the constructor specified by the constructor index. Return the ConstructorArgs object which
     * contains the actual arguments to the alternate constructor and also the index of that constructor for the constructor switch.
     *
     * @param consArgs the arguments to the constructor. These are necessary in the evaluation of the alt constructor args. e.g. Foo(a) {
     *                 super(a); }
     * @return the ConstructorArgs object containing a constructor selector and evaluated arguments for the alternate constructor
     */
    @SuppressWarnings("rawtypes")
    ...
    /**
     * Register actual context, used by generated class constructor, which calls {@link #initInstance(GeneratedClass, String, Object[])}.
     */
    ...
    /**
     * Initialize an instance of the class. This method is called from the generated class constructor to evaluate the instance initializer
     * and scripted constructor in the instance namespace.
     */
    @SuppressWarnings("rawtypes")
    ...
    public byte[] generateClass() {
        ...
        for (int i = 0; i < interfaces.length; i++) {
            interfaceNames[i] = org.objectweb.asm.Type.getInternalName(interfaces[i]);
        }
        if (!isInterface) {
     ...
     */
    void generateConstructor(int index, String[] paramTypes, int modifiers, org.objectweb.asm.ClassWriter cw) {
        ...
        org.objectweb.asm.CodeVisitor cv = cw.visitMethod(modifiers, "<init>", methodDescriptor, exceptions);
    ...
    /**
     * Generate a switch with a branch for each possible alternate constructor. This includes all superclass constructors and all
     * constructors of this class. The default branch of this switch is the default superclass constructor.
     * <p/>
     * This method also generates the code to call the static ClassGeneratorUtil getConstructorArgs() method which inspects the scripted
     * constructor to find the alternate constructor signature (if any) and evalute the arguments at runtime. The getConstructorArgs()
     * method returns the actual arguments as well as the index of the constructor to call.
     */
    void generateConstructorSwitch(int consIndex, int argsVar, int consArgsVar, org.objectweb.asm.CodeVisitor cv) {
        ...
        org.objectweb.asm.Label[] labels = new org.objectweb.asm.Label[cases];
    ...
    /**
     * A ConstructorArgs object holds evaluated arguments for a constructor call as well as the index of a possible alternate selector to
     * invoke. This object is used by the constructor switch.
     *
     * @see ClassGeneratorUtil#generateConstructor(int, String[], int, org.objectweb.asm.ClassWriter)
     */
