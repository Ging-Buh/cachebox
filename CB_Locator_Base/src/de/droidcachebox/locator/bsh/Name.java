...
class Name implements java.io.Serializable {
    ...
    /**
     * @param forceClass if true then resolution will only produce a class. This is necessary to disambiguate in cases where the grammar knows that
     ...
     * <p/>
     *
     * @param callstack may be null, but this is only legitimate in special cases where we are sure resolution will not involve this.caller.
    ...
    public Object invokeMethod(Interpreter interpreter, Object[] args, CallStack callstack, SimpleNode callerInfo) throws UtilEvalError, EvalError, ReflectError, InvocationTargetException {
        ...
        if (!(obj instanceof ClassIdentifier)) {
            ...
            if (obj instanceof Primitive) {
                ...
                if (Interpreter.DEBUG)
                    Interpreter.debug("Attempt to access method on primitive..." + " allowing de.droidcachebox.locator.bsh.Primitive to peek through for debugging");
            }
