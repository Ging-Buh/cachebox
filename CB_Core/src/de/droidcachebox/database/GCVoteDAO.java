...
public class GCVoteDAO {
    ...
    public int getCacheCountToGetVotesFor(String whereClause) {
        ...
        CoreCursor reader = CBDB.Data.sql.rawQuery("select count(GcCode) from Caches " + ((whereClause.length() > 0) ? "where " + whereClause : whereClause), null);
    ...
    public ArrayList<GCVoteCacheInfo> getGCVotePackage(String whereClause, int packagesize, int offset) {
        ...
        CoreCursor reader = CBDB.Data.sql.rawQuery("select Id, GcCode, VotePending from Caches " + ((whereClause.length() > 0) ? "where " + whereClause : whereClause) + " LIMIT " + offset + "," + packagesize, null);
    ...
    public void updateRatingAndVote(Long Id, Float Rating, Float Vote) {
        ...
        CBDB.Data.sql.update("Caches", parm, "Id=?", new String[]{String.valueOf(Id)});
    }
    ...
    public ArrayList<GCVoteCacheInfo> getPendingGCVotes() {
        ...
        CoreCursor reader = CBDB.Data.sql.rawQuery("select Id, GcCode, Url, Vote from Caches where VotePending=1", null);
