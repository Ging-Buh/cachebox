...
public abstract class CBDB extends Database_Core {
    ...
    public void deleteFromDatabase(Waypoint waypoint) {
        ...
        try {
            sql.delete("Waypoint", "GcCode='" + waypoint.getWaypointCode() + "'", null);
        } catch (Exception exc) {
    ...
    public void WriteLogEntry(LogEntry logEntry) {
        ...
        try {
            CBDB.Data.sql.insertWithConflictReplace("Logs", args);
        } catch (Exception exc) {
    ...
    private boolean waypointExists(String gcCode) {
        CoreCursor c = sql.rawQuery("select GcCode from Waypoint where GcCode=@gccode", new String[]{gcCode});
        {
            c.moveToFirst();
            if (!c.isAfterLast()) {
                try {
    ...
    public String createFreeGcCode(String cacheGcCode) throws Exception {
        ...
        for (int i = 0; i < firstCharCandidates.length(); i++)
            for (int j = 0; j < secondCharCandidates.length(); j++) {
                ...
                if (!CBDB.Data.waypointExists(gcCode))
                    return gcCode;
            }
            ...
            if (lastDatabaseSchemeVersion < 1015) {
                ...
                for (Entry<Long, String> entry : gpxFilenames.entrySet()) {
                    ...
                    if (categories.containsKey(entry.getValue())) {
                        ...
                        try {
                            CBDB.Data.sql.update("GpxFilenames", args, "Id=" + entry.getKey(), null);
                        } catch (Exception exc) {
    ...
    public int getCacheCountInDB() {
        ...
        int count = 0;
        try {
            reader = CBDB.Data.sql.rawQuery("select count(*) from caches", null);
            reader.moveToFirst();
    ...
    public void deleteOldLogs(int minToKeep, int LogMaxMonthAge) {
        ...
        // #############################################################################
        {
            try {
                ...
                CoreCursor reader = CBDB.Data.sql.rawQuery(command, null);
                reader.moveToFirst();
                while (!reader.isAfterLast()) {
        ...
        // ###################################################
        {
            try {
                ...
                for (long oldLogCache : oldLogCaches) {
                    ...
                    CBDB.Data.sql.execSQL(delCommand);
                }
                sql.setTransactionSuccessful();
            } catch (Exception ex) {
                Log.err(log, "deleteOldLogs", ex);
