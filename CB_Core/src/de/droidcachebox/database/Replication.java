...
public class Replication {
    ...
    public static void FoundChanged(long CacheId, boolean found) {
        if (CBDB.Data.MasterDatabaseId > 0) {
            ...
            CBDB.Data.sql.delete("Replication", "CacheId=" + String.valueOf(CacheId) + " and ChangeType=6 or CacheId=" + String.valueOf(CacheId) + " and ChangeType=7", null);

            Parameters val = new Parameters();
            val.put("CacheId", CacheId);
            val.put("ChangeType", changeType.ordinal());
            CBDB.Data.sql.insert("Replication", val);
        }
        ...
        if (CBDB.Data.MasterDatabaseId > 0) {
            if (oldSolverCheckSum != newSolverCheckSum) {
                if (oldSolverCheckSum != newSolverCheckSum) {
                    ...
                    // Text
                    int dbCheckSum = -1;
                    CoreCursor c = CBDB.Data.sql.rawQuery("select SolverCheckSum from Replication where CacheId=? and ChangeType=?", new String[]{String.valueOf(CacheId), String.valueOf(ChangeType.SolverText.ordinal())});
                    c.moveToFirst();
                    while (!c.isAfterLast()) {
                    ...
                    if (dbCheckSum < 0) {
                        ...
                        if (dbCheckSum == newSolverCheckSum) {
                            ...
                            // -> delete this entry!
                            CBDB.Data.sql.delete("Replication", "CacheId=? and ChangeType=?", new String[]{String.valueOf(CacheId), String.valueOf(ChangeType.SolverText.ordinal())});
                        }
                    }
                }
            }
        ...
        if (CBDB.Data.MasterDatabaseId > 0) {
            if (oldNoteCheckSum != newNoteCheckSum) {
                if (oldNoteCheckSum != newNoteCheckSum) {
                    ...
                    // Text
                    int dbCheckSum = -1;
                    CoreCursor c = CBDB.Data.sql.rawQuery("select NotesCheckSum from Replication where CacheId=? and ChangeType=?", new String[]{String.valueOf(CacheId), String.valueOf(ChangeType.NotesText.ordinal())});
                    c.moveToFirst();
                    while (!c.isAfterLast()) {
                        dbCheckSum = c.getInt(0);
                        break;
                    ...
                    if (dbCheckSum < 0) {
                        ...
                        if (dbCheckSum == newNoteCheckSum) {
                            ...
                            // -> delete this entry!
                            CBDB.Data.sql.delete("Replication", "CacheId=? and ChangeType=?", new String[]{String.valueOf(CacheId), String.valueOf(ChangeType.NotesText.ordinal())});
                        }
                    }
                }
            }
    ...
    private static void Changed(long CacheId, int oldCheckSum, int newCheckSum, String checkSumType, ChangeType changeType, String WpGcCode) {
        if (CBDB.Data.MasterDatabaseId > 0) {
            if (oldCheckSum != newCheckSum) {
                ...
                int dbCheckSum = -1;
                CoreCursor c = CBDB.Data.sql.rawQuery("select " + checkSumType + " from Replication where CacheId=? and ChangeType=? and WpGcCode=?", new String[]{String.valueOf(CacheId), String.valueOf(changeType.ordinal()), WpGcCode});
                c.moveToFirst();
                while (!c.isAfterLast()) {
                    dbCheckSum = c.getInt(0);
                    break;
                ...
                if (dbCheckSum < 0) {
                    ...
                    if (dbCheckSum == newCheckSum) {
                        ...
                        // -> delete this entry!
                        CBDB.Data.sql.delete("Replication", "CacheId=? and ChangeType=?", new String[]{String.valueOf(CacheId), String.valueOf(changeType)});
                    }
                }
            }
        }
    ...
    public static void AvailableChanged(long CacheId, boolean available) {
        if (CBDB.Data.MasterDatabaseId > 0) {
            ...
            CBDB.Data.sql.delete("Replication", "CacheId=" + String.valueOf(CacheId) + " and ChangeType=10 or CacheId=" + String.valueOf(CacheId) + " and ChangeType=11", null);
    ...
    public static void ArchivedChanged(long CacheId, boolean archived) {
        if (CBDB.Data.MasterDatabaseId > 0) {
            ...
            CBDB.Data.sql.delete("Replication", "CacheId=" + String.valueOf(CacheId) + " and ChangeType=8 or CacheId=" + String.valueOf(CacheId) + " and ChangeType=9", null);
    ...
    public static void NumTravelbugsChanged(long CacheId, int numTravelbugs) {
        if (CBDB.Data.MasterDatabaseId > 0) {
            ...
            CBDB.Data.sql.delete("Replication", "CacheId=" + String.valueOf(CacheId) + " and ChangeType=12", null);
    ...
    public static void NumFavPointsChanged(long CacheId, int numFavPoints) {
        if (CBDB.Data.MasterDatabaseId > 0) {
            ...
            CBDB.Data.sql.delete("Replication", "CacheId=" + String.valueOf(CacheId) + " and ChangeType=" + changeType.ordinal(), null);
