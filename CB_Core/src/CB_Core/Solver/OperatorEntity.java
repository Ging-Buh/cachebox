...
class OperatorEntity extends Entity
{
	...
	public String Berechne()
	{
		...
		try
		{
			...
			if (op.equals("+")) result = DoubleToString(dLinks + dRechts);
			else if (op.equals("-")) result = DoubleToString(dLinks - dRechts);
			else if (op.equals("*")) result = DoubleToString(dLinks * dRechts);
			else if (op.equals("/")) result = DoubleToString(dLinks / dRechts);
			else if (op.equals(":"))
			{
			...
			else if (op.equals("^")) result = DoubleToString(Math.pow(dLinks, dRechts));
		}
		catch (Exception ex)
		{
			// Fehler ausgeben.
