...
public class DescriptionImageGrabber
{
			...
			if (srcIdx != -1 && srcStart != -1 && srcEnd != -1)
			{
				...
				try
				{
					...
					else
					{
						...
						{
							// Wenn nicht-lokale Inhalte unterdr�ckt werden
							// sollen,
							// wird das <img>-Tag vollst�ndig entfernt
	...
	public static int GrabImagesSelectedByCache(ImporterProgress ip, boolean descriptionImagesUpdated, boolean additionalImagesUpdated, long id, String gcCode, String name, String description, String url)
	{
		...
		if (!additionalImagesUpdated)
		{
			...
			// Liste aller Spoiler Images f�r diesen Cache erstellen
			// anhand dieser Liste kann �berpr�ft werden, ob ein Spoiler schon geladen ist und muss nicht ein 2. mal geladen werden.
			// Au�erdem k�nnen anhand dieser Liste veraltete Spoiler identifiziert werden, die gel�scht werden k�nnen / m�ssen
...

			{
				...
				do
				{
					...
					if (result == 140)
					{
						// API-Limit �berschritten -> nach 15 Sekunden wiederholen
						System.out.println("******* API-Limit �berschritten -> 15 Sekunden warten! *******");
						try
						{
							Thread.sleep(15000);
						}
						...
						if (System.currentTimeMillis() > startTs + 60000)
						{
							...
							System.out.println("******* Timeout API-Limit �berschritten ********");
							break;
						}
					}
					else
				...
				for (String key : allimgDict.keySet())
				{
					...
					// Neuen Local Filename mit Hash erzeugen, damit �nderungen der Datei ohne �nderungen des Dateinamens erkannt werden
					// k�nnen
				...
				if (imageLoadError == false)
				{
					...
					for (String file : afiles)
					{
						...
						// Testen, ob dieser Dateiname einen g�ltigen ACB Hash hat (eingeschlossen zwischen @....@>
						if (fileNameWithOutExt.endsWith("@") && fileNameWithOutExt.contains("@"))
						{
							// file enth�lt nur den Dateinamen, nicht den Pfad. Diesen Dateinamen um den Pfad erweitern, in dem hier die
	...
	/**
	 * Neue Version, mit @ als Eingrenzung des Hashs, da die Klammern nicht als URL's verwendet werden d�rfen
	 * 
	 * @param GcCode
	 * @param ImageName
	 * @param uri
