...
public class CacheListLite extends MoveableList<CacheLite>
{
	...
	public CacheLite GetCacheByGcCode(String GcCode)
	{
	...
	public CacheLite GetCacheById(long cacheId)
	{
		for (int i = 0, n = this.size(); i < n; i++)
		{
			CacheLite cache = this.get(i);
	...
	public CacheWithWP Resort(Coordinate selectedCoord, CacheWithWP selected)
	{
		...
		if (this.size() > 0)
		{
			CacheLite nextCache = this.get(0); // or null ...
			...
			// read FullCache from DB!
			CacheDAO dao = new CacheDAO();
			Cache fullCache = dao.getFromDbByCacheId(nextCache.Id);

			retValue = new CacheWithWP(fullCache, waypoint);
		}

		CB_Core.Events.CachListChangedEventList.Call();

		// vorhandenen Parkplatz Cache nach oben schieben
		CacheLite park = this.GetCacheByGcCode("CBPark");
		if (park != null)
		{
