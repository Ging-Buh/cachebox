...
public class computeDistanceAndBearing extends TestCase
{
	...
	void compute(Coordinate finalCoord)
	{
		...
		MathUtils.computeDistanceAndBearing(ACCURATE, position.getLatitude(), position.getLongitude(), finalCoord.getLatitude(),
				finalCoord.getLongitude(), result);

		float result2[] = new float[4];
		MathUtils.computeDistanceAndBearing(FAST, position.getLatitude(), position.getLongitude(), finalCoord.getLatitude(),
				finalCoord.getLongitude(), result2);
		...
		if (div < 0) div *= -1;
		assertFalse("Distance Abweichung zwichen ACCURATE und FAST zu Groß", div > maxabweichung);

		double BearingDiv = result[1] - result2[1];
		if (BearingDiv < 0) BearingDiv *= -1;
		assertFalse("Bearing Abweichung zwichen ACCURATE und FAST zu Groß", BearingDiv > 0.1f);
