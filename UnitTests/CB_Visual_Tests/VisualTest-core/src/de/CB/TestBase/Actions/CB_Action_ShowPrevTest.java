...
public class CB_Action_ShowPrevTest extends CB_Action_ShowView
{
	...
	public void Execute()
	{
		...
		if (CB_Action_ShowNextTest.testList.size() == 0) return; // no more tests available
		...
		if ((tab != null)) tab.ShowView(CB_Action_ShowNextTest.actTest);

		// dispose last
		if (CB_Action_ShowNextTest.actTestIndex < CB_Action_ShowNextTest.testList.size() - 1) CB_Action_ShowNextTest.testList.get(
				CB_Action_ShowNextTest.actTestIndex + 1).dispose();
		System.gc();
	}
