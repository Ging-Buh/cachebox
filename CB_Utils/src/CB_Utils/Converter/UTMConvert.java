...
public class UTMConvert
{
	...
	public int iLatLon2UTM(double dLat, double dLon)
	{
		...
		double M = dEquatorialRadius
				* ((1 - eccSquared / 4 - 3 * eccSquared * eccSquared / 64 - 5 * eccSquared * eccSquared * eccSquared / 256) * dLatRad
						...
						* Math.sin(4 * dLatRad) - (35 * eccSquared * eccSquared * eccSquared / 3072) * Math.sin(6 * dLatRad));

		UTMEasting = (double) (dScaleFactor * N
				* (A + (1 - T + C) * A * A * A / 6 + (5 - 18 * T + T * T + 72 * C - 58 * eccPrimeSquared) * A * A * A * A * A / 120) + 500000.0);

		UTMNorthing = (double) (dScaleFactor * (M + N
				* Math.tan(dLatRad)
				* (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * A * A * A * A / 24 + (61 - 58 * T + T * T + 600 * C - 330 * eccPrimeSquared)
						* A * A * A * A * A * A / 720)));
		if (dLat < 0) UTMNorthing += 10000000.0; // 10000000 meter offset for southern hemisphere
	...
	public int iUTM2LatLon(double UTMNorthing, double UTMEasting, String sUTMZone)
	{
		...
		// phi in radians
		double phi1Rad = mu + (3 * e1 / 2 - 27 * e1 * e1 * e1 / 32) * Math.sin(2 * mu) + (21 * e1 * e1 / 16 - 55 * e1 * e1 * e1 * e1 / 32)
		...
		dLat = phi1Rad
				...
				* (D * D / 2 - (5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * eccPrimeSquared) * D * D * D * D / 24 + (61 + 90 * T1 + 298 * C1
	...
	public int iGetUtmZone(double dLat, double dLon)
	{
		...
		{
			if (dLon >= 0.0 && dLon < 9.0) iUTM_Zone_Num = 31;
			else if (dLon >= 9.0 && dLon < 21.0) iUTM_Zone_Num = 33;
			else if (dLon >= 21.0 && dLon < 33.0) iUTM_Zone_Num = 35;
			else if (dLon >= 33.0 && dLon < 42.0) iUTM_Zone_Num = 37;
		}
