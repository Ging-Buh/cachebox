...
public class QualityPanel extends JPanel implements GPSDataListener {
	...
	private JComboBox satellites = new JComboBox();
	...
	private JComboBox fixType = new JComboBox();
	
	// spinners for DOP parameters
	private JSpinner hdop = new JSpinner();
	private JSpinner vdop = new JSpinner();
	private JSpinner pdop = new JSpinner();
	
	// checkbox whether pdop should depend on hdop and vdop
	private JCheckBox automaticPDOP = new JCheckBox();
	
	// checkbox whether status should depend on number of satellites
	private JCheckBox automaticStatus = new JCheckBox();
	...
	public QualityPanel() {
		...
		for (int i = 0; i <= 12; i++) { satellites.addItem(i); }
		satellites.setSelectedIndex(GPSData.getSatellites());
