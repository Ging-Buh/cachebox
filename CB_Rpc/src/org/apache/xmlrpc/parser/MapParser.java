		...
		if (inName) {
			...
			if (nameObject == null) {
				...
				if (map.containsKey(nameObject)) {
					throw new SAXParseException("Duplicate name: " + nameObject, getDocumentLocator());
				} else {
	...
	public void startElement(String pURI, String pLocalName, String pQName, Attributes pAttrs) throws SAXException {
		...
		case 0:
			if (!"".equals(pURI) || !MapSerializer.STRUCT_TAG.equals(pLocalName)) {
				throw new SAXParseException("Expected " + MapSerializer.STRUCT_TAG + ", got " + new QName(pURI, pLocalName), getDocumentLocator());
			}
			break;
		case 1:
			if (!"".equals(pURI) || !MapSerializer.MEMBER_TAG.equals(pLocalName)) {
				throw new SAXParseException("Expected " + MapSerializer.MEMBER_TAG + ", got " + new QName(pURI, pLocalName), getDocumentLocator());
			}
		...
		case 2:
			...
			if ("".equals(pURI) && MapSerializer.NAME_TAG.equals(pLocalName)) {
				if (nameObject == null) {
					...
					throw new SAXParseException("Expected " + TypeSerializerImpl.VALUE_TAG + ", got " + new QName(pURI, pLocalName), getDocumentLocator());
				}
			} else if ("".equals(pURI) && TypeSerializerImpl.VALUE_TAG.equals(pLocalName)) {
				if (nameObject == null) {
					throw new SAXParseException("Expected " + MapSerializer.NAME_TAG + ", got " + new QName(pURI, pLocalName), getDocumentLocator());
				} else {
					inValue = true;
					startValueTag();
				}
