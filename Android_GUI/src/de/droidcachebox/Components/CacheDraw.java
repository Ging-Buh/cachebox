...
public class CacheDraw
{
	...
	public static void DrawInfo(Cache cache, Canvas canvas, int width,
			int height, int BackgroundColor, DrawStyle drawStyle)
	{
		int x = 0;
		int y = 0;
	...
	public static void DrawInfo(Cache cache, Canvas canvas, Rect rec,
			int BackgroundColor, DrawStyle drawStyle, float scale)
	{
		if (CachedBitmap == null || !(CachedBitmapId == cache.Id))
		{
			CachedBitmap = Bitmap.createBitmap(rec.width(), rec.height(),
			...
			DrawInfo(cache, scaledCanvas, newRec, BackgroundColor, Color.RED,
					drawStyle,false);
		...
		canvas.drawBitmap(CachedBitmap, rec.left / scale, rec.top / scale,
				CachedBitmapPaitnt);
	...
	public static void DrawInfo(Cache cache, Canvas canvas, Rect rec,
			int BackgroundColor, DrawStyle drawStyle, Boolean withoutBearing)
	{
		DrawInfo(cache, canvas, rec, BackgroundColor, -1, drawStyle,withoutBearing);
	}

	public static void DrawInfo(Cache cache, Canvas canvas, Rect rec,
			int BackgroundColor, int BorderColor, DrawStyle drawStyle, Boolean withoutBearing)
	{
		// init
		...
		if (BackgroundColor == -1) BackgroundColor = GlobalSelected ? Global
				.getColor(R.attr.ListBackground_select) : Global
				.getColor(R.attr.ListBackground);
		if (BorderColor == -1) BorderColor = Global
				.getColor(R.attr.ListSeparator);
		...
		final int SDTImageTop = (int) (height - (Sizes
				.getScaledFontSize_normal() / 1.5)) + rec.top - 10;
		...
		if (VoteWidth == 0) // Grössen noch nicht berechnet
		{
			...
			nameLayoutWidthRightBorder = width - VoteWidth
					- Sizes.getIconSize() - rightBorder
					- (Sizes.getScaledFontSize_normal() / 2);
			nameLayoutWidth = width - VoteWidth - Sizes.getIconSize()
					- (Sizes.getScaledFontSize_normal() / 2);
			DTPaint = new Paint();
		...
		// Draw roundetRect
		ActivityUtils.drawFillRoundRecWithBorder(canvas, rec, 2, BorderColor,
				BackgroundColor);

		// Draw Vote
		if (cache.Rating > 0) ActivityUtils.PutImageScale(canvas,
				Global.StarIcons[(int) (cache.Rating * 2)], -90, left, top,
		...
		if (cache.MysterySolved())
		{
			ActivityUtils.PutImageTargetHeight(canvas,
					Global.CacheIconsBig[19], left + VoteWidth, top
							- (int) (Sizes.getScaledFontSize_normal() / 2),
					Sizes.getIconSize());
		}
		else
		{
			ActivityUtils.PutImageTargetHeight(canvas,
					Global.CacheIconsBig[cache.Type.ordinal()], left
							+ VoteWidth,
					top - (int) (Sizes.getScaledFontSize_normal() / 2),
					Sizes.getIconSize());
		}
		...
		String drawName = (drawStyle == DrawStyle.withOwner) ? "by "
				+ cache.Owner + ", " + dateString  : cache.Name;
		...
		layoutCacheName = new StaticLayout(drawName, namePaint,
				nameLayoutWidthRightBorder, Alignment.ALIGN_NORMAL, 1.0f, 0.0f,
				false);
		int LayoutHeight = ActivityUtils.drawStaticLayout(canvas,
				layoutCacheName, left + VoteWidth + Sizes.getIconSize() + 5,
				top);
...
//			backPaint.setColor(Color.RED); //DEBUG
			...
			canvas.drawRect(new Rect(
					left + VoteWidth + Sizes.getIconSize() + 5, SDTImageTop, nameLayoutWidthRightBorder + left
							+ VoteWidth + Sizes.getIconSize() + 5, top
							+ LayoutHeight + VislinesHeight - 4), backPaint);
		}
		...
		if (drawStyle == DrawStyle.withOwnerAndName)
		{
			...
			{
				DrawText = "by "
						+ cache.Owner.substring(0, cache.Owner.length()
								- counter) + ", " + dateString;
				counter++;
			}
			...
			DrawText = DrawText + String.format("%n")+ String.format("%n") + cache.Pos.FormatCoordinate()+ String.format("%n") + cache.GcCode + String.format("%n") ;
			...
			layoutCacheOwner = new StaticLayout(DrawText, namePaint,
			...
			ActivityUtils
					.drawStaticLayout(canvas, layoutCacheOwner, left
							+ VoteWidth + Sizes.getIconSize() + 5, top
							+ (VislinesHeight/2));
		}
		...
		SDTleft += ActivityUtils.PutImageTargetHeight(canvas,
				Global.SizeIcons[(int) (cache.Size.ordinal())], SDTleft,
				SDTImageTop, Sizes.getScaledFontSize_normal());
		SDTleft += Sizes.getTabWidth();
		canvas.drawText("D", SDTleft, SDTLineTop, DTPaint);
		SDTleft += Sizes.getSpaceWidth();
		SDTleft += ActivityUtils.PutImageTargetHeight(canvas,
				Global.StarIcons[(int) (cache.Difficulty * 2)], SDTleft,
				SDTImageTop, Sizes.getScaledFontSize_normal());
		SDTleft += Sizes.getTabWidth();
		canvas.drawText("T", SDTleft, SDTLineTop, DTPaint);
		SDTleft += Sizes.getSpaceWidth();
		SDTleft += ActivityUtils.PutImageTargetHeight(canvas,
				Global.StarIcons[(int) (cache.Terrain * 2)], SDTleft,
				SDTImageTop, Sizes.getScaledFontSize_normal());
		SDTleft += Sizes.getTabWidth();
		...
		if (numTb > 0)
		{
			SDTleft += ActivityUtils.PutImageScale(canvas, Global.Icons[0],
					-90, SDTleft, SDTImageTop - 1,
					(double) Sizes.getScaledFontSize_normal() / 30);
			// SDTleft += space;
			if (numTb > 1) canvas.drawText("x" + String.valueOf(numTb),
					SDTleft, SDTLineTop, DTPaint);
		}

		// Draw Bearing

		if (drawStyle != DrawStyle.withoutBearing
				&& drawStyle != DrawStyle.withOwnerAndName && !withoutBearing)
		{
			if (BearingRec == null) BearingRec = new Rect(rec.right
					- rightBorder, rec.top, rec.right,
					(int) (SDTImageTop * 0.9));
		...
		if (cache.Found)
		{
			...
			ActivityUtils.PutImageTargetHeight(
					canvas,
					Global.Icons[2],
					left + VoteWidth + Sizes.getIconSize() / 2,
					top - (int) (Sizes.getScaledFontSize_normal() / 2)
							+ Sizes.getIconSize() / 2, Sizes.getIconSize() / 2);// Smile
		}

		if (cache.Favorit())
		{
			ActivityUtils.PutImageTargetHeight(canvas, Global.Icons[19], left
					+ VoteWidth + 2, top, Sizes.getIconSize() / 2);
		}

		if (cache.Archived)
		{
			ActivityUtils.PutImageTargetHeight(canvas, Global.Icons[24], left
					+ VoteWidth + 2, top, Sizes.getIconSize() / 2);
		}
		...
		if (cache.ImTheOwner())
		{
			ActivityUtils.PutImageTargetHeight(
					...
					left + VoteWidth + Sizes.getIconSize() / 2,
					top - (int) (Sizes.getScaledFontSize_normal() / 2)
							+ Sizes.getIconSize() / 2, Sizes.getIconSize() / 2);
		}
	...
	private static void DrawBearing(Cache cache, Canvas canvas, Rect drawingRec)
	{
		...
		{
			Coordinate position = (GlobalCore.Marker.Valid) ? GlobalCore.Marker
					: GlobalCore.LastValidPosition;
			double heading = (Global.Locator != null) ? Global.Locator
					.getHeading() : 0;
			double bearing = Coordinate.Bearing(position.Latitude,
					position.Longitude, cache.Latitude(), cache.Longitude());
			double cacheBearing = bearing - heading;
			String cacheDistance = UnitFormatter.DistanceString(cache
					.Distance(false));
	...
	public void DrawBearing(Cache cache, Canvas canvas, Rect drawingRec,
			Waypoint waypoint)
	{
		if (GlobalCore.LastValidPosition.Valid || GlobalCore.Marker.Valid)
		{
			...
			double heading = (Global.Locator != null) ? Global.Locator
					.getHeading() : 0;
			double bearing = Coordinate.Bearing(position.Latitude,
					position.Longitude, waypoint.Latitude(),
					waypoint.Longitude());
			double waypointBearing = bearing - heading;
			float distance = 0;
			if (waypoint == null) distance = cache.Distance(false);
			else
				distance = waypoint.Distance();
			String waypointDistance = UnitFormatter.DistanceString(distance);
			DrawBearing(cache, canvas, drawingRec, waypointDistance,
					waypointBearing);

		}
	}

	private static void DrawBearing(Cache cache, Canvas canvas,
			Rect drawingRec, String Distance, double Bearing)
	{

		double scale = (double) Sizes.getScaledFontSize_normal() / 30;

		ActivityUtils.PutImageScale(canvas, Global.Arrows[1], Bearing,
				drawingRec.left, drawingRec.top, scale);
