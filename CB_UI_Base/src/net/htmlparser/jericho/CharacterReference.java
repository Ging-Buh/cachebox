...
/**
 * Represents an HTML <a target="_blank" href="http://www.w3.org/TR/REC-html40/charset.html#entities">Character Reference</a>, implemented
 * by the subclasses {@link CharacterEntityReference} and {@link NumericCharacterReference}.
 * <p>
 * This class, together with its subclasses, contains static methods to perform most required operations without having to instantiate an
 * object.
 * <p>
 * Instances of this class are useful when the positions of character references in a source document are required, or to replace the found
 * character references with customised text.
 * <p>
...
public abstract class CharacterReference extends Segment {
	 ...
	 * <p>
	 * This can be the result of parsing a numeric character reference outside of the valid unicode range of 0x000000-0x10FFFF, or any other
	 ...
	 * <p>
	 * If this character reference represents a unicode <a target="_blank"
	 * href="http://www.unicode.org/glossary/#supplementary_code_point">supplimentary code point</a>, any bits outside of the least
	 * significant 16 bits of the code point are truncated, yielding an incorrect result.
	 * <p>
	 ...
	 * <br />
	 * <code>characterReference.</code>{@link #appendCharTo(Appendable) appendCharTo}<code>(appendable)</code><br />
	 * instead of: <br />
	 * <code>appendable.append(characterReference.getChar())</code>
	 *
	 ...
	 * <p>
	 * If this character is a unicode <a target="_blank" href="http://unicode.org/glossary/#supplementary_character">supplementary
	 * character</a>, then both the UTF-16 high/low surrogate <code>char</code> values of the of the character are appended, as described in
	 * the <a target="_blank" href="http://java.sun.com/javase/6/docs/api/java/lang/Character.html#unicode">Unicode character
	 * representations</a> section of the <code>java.lang.Character</code> class.
	 * <p>
	 * If the static {@link Config#ConvertNonBreakingSpaces} property is set to <code>true</code> (the default), then calling this method on
	 * a non-breaking space character reference ({@link CharacterEntityReference#_nbsp &amp;nbsp;}) results in a normal space being
	 * appended.
	 *
	 * @param appendable
	 ...
	 * <p>
	 * Conversely, this library defines an <i><a name="Unterminated">unterminated</a></i> character reference as one which does not end with
	 * a semicolon.
	 * <p>
	 * The SGML specification allows unterminated character references in some circumstances, and because the HTML 4.01 specification states
	 * simply that
	 * "<a target="_blank" href="http://www.w3.org/TR/REC-html40/charset.html#entities">authors may use SGML character references</a>", it
	 * follows that they are also valid in HTML documents, although their use is strongly discouraged.
	 * <p>
	 ...
	 * <p>
	 * Each character is encoded only if the {@link #requiresEncoding(char)} method would return <code>true</code> for that character, using
	 * its {@link CharacterEntityReference} if available, or a decimal {@link NumericCharacterReference} if its unicode code point is
	 * greater than U+007F.
	 * <p>
	 * The only exception to this is an {@linkplain CharacterEntityReference#_apos apostrophe} (U+0027), which depending on the current
	 * setting of the static {@link Config#IsApostropheEncoded} property, is either left unencoded (default setting), or encoded as the
	 * numeric character reference "<code>&amp;#39;</code>".
	 * <p>
	 * This method never encodes an apostrophe into its character entity reference {@link CharacterEntityReference#_apos &amp;apos;} as this
	...
	/**
	 * {@linkplain #encode(CharSequence) Encodes} the specified text, preserving line breaks, tabs and spaces for rendering by converting
	 * them to markup.
	 * <p>
	 * This performs the same encoding as the {@link #encode(CharSequence)} method, but also performs the following conversions:
	 * <ul>
	 * <li>Line breaks, being Carriage Return (U+000D) or Line Feed (U+000A) characters, and Form Feed characters (U+000C) are converted to
	 * "<code>&lt;br /&gt;</code>". CR/LF pairs are treated as a single line break.
	 * <li>Multiple consecutive spaces are converted so that every second space is converted to "<code>&amp;nbsp;</code>" while ensuring the
	 * last is always a normal space.
	 * <li>Tab characters (U+0009) are converted as if they were four consecutive spaces.
	 * </ul>
	 * <p>
	 * The conversion of multiple consecutive spaces to alternating space/non-breaking-space allows the correct number of spaces to be
	 * rendered, but also allows the line to wrap in the middle of it.
	 * <p>
	 * Note that zero-width spaces (U+200B) are converted to the numeric character reference "<code>&amp;#x200B;</code>" through the normal
	 * encoding process, but IE6 does not render them properly either encoded or unencoded.
	 * <p>
	 ...
	 * <p>
	 * All {@linkplain CharacterEntityReference character entity references} and {@linkplain NumericCharacterReference numeric character
	 * references} are converted to their respective characters.
	 * <p>
	 * This is equivalent to {@link #decode(CharSequence,boolean) decode(encodedText,false)}.
	 * <p>
	 * <a href="#Unterminated">Unterminated</a> character references are dealt with according to the rules for text outside of attribute
	 * values in the {@linkplain Config#CurrentCompatibilityMode current compatibility mode}.
	 * <p>
	 * If the static {@link Config#ConvertNonBreakingSpaces} property is set to <code>true</code> (the default), then all non-breaking space
	 * ({@link CharacterEntityReference#_nbsp &amp;nbsp;}) character entity references are converted to normal spaces.
	 * <p>
	 * Although character entity reference names are case sensitive, and in some cases differ from other entity references only by their
	 * case, some browsers also recognise them in a case-insensitive way. For this reason, all decoding methods in this library recognise
	 ...
	 * references} are converted to their respective characters.
	 * <p>
	 * <a href="#Unterminated">Unterminated</a> character references are dealt with according to the value of the
	 * <code>insideAttributeValue</code> parameter and the {@linkplain Config#CurrentCompatibilityMode current compatibility mode}.
	 * <p>
	 * If the static {@link Config#ConvertNonBreakingSpaces} property is set to <code>true</code> (the default), then all non-breaking space
	 * ({@link CharacterEntityReference#_nbsp &amp;nbsp;}) character entity references are converted to normal spaces.
	 * <p>
	 ...
	 * @param encodedText
	 *            the text to decode.
	 * @param insideAttributeValue
	 *            specifies whether the encoded text is inside an attribute value.
	 * @return the decoded string.
	 * @see #decode(CharSequence)
	 * @see #encode(CharSequence)
	 */
	 ...
	 * <p>
	 * The result is how the text would normally be rendered by a <a target="_blank"
	 * href="http://www.w3.org/TR/html401/conform.html#didx-user_agent">user agent</a>, assuming it does not contain any tags.
	 * <p>
	 * If the static {@link Config#ConvertNonBreakingSpaces} property is set to <code>true</code> (the default), then all non-breaking space
	 * ({@link CharacterEntityReference#_nbsp &amp;nbsp;}) character entity references are converted to normal spaces.
	 * <p>
	 * <a href="#Unterminated">Unterminated</a> character references are dealt with according to the rules for text outside of attribute
	 * values in the {@linkplain Config#CurrentCompatibilityMode current compatibility mode}. See the discussion of the
	 * <code>insideAttributeValue</code> parameter of the {@link #decode(CharSequence, boolean insideAttributeValue)} method for a more
	...
	/**
	 * Re-encodes the specified text, equivalent to {@linkplain #decode(CharSequence) decoding} and then {@linkplain #encode(CharSequence)
	 * encoding} again.
	 * <p>
	 ...
	 * {@link #encode(CharSequence) encode} method, but a more efficient implementation may be used in future.
	 *
	 * @param encodedText
	 ...
	 * <p>
	 * The exact behaviour of this method depends on the class of this object. See the
	 * {@link CharacterEntityReference#getCharacterReferenceString()} and {@link NumericCharacterReference#getCharacterReferenceString()}
	 ...
	 * <p>
	 * This method returns the {@linkplain CharacterEntityReference#getCharacterReferenceString(int) character entity reference} encoded
	 * form of the unicode code point if one exists, otherwise it returns the {@linkplain #getDecimalCharacterReferenceString(int) decimal
	 * character reference} encoded form.
	 * <p>
	 * The only exception to this is an {@linkplain CharacterEntityReference#_apos apostrophe} (U+0027), which is encoded as the numeric
	 * character reference "<code>&amp;#39;</code>" instead of its character entity reference "<code>&amp;apos;</code>".
	 * <p>
	 * <dl>
	 * <dt>Examples:</dt>
	 ...
	 * <p>
	 * This is equivalent to {@link #getDecimalCharacterReferenceString(int) getDecimalCharacterReferenceString}<code>(</code>
	...
	/**
	 * Returns the <a href="NumericCharacterReference.html#DecimalCharacterReference">decimal encoded form</a> of the specified unicode code
	 * point.
	 * <p>
	 * <dl>
	 * <dt>Example:</dt>
	...
	/**
	 * Returns the <a href="NumericCharacterReference.html#HexadecimalCharacterReference">hexadecimal encoded form</a> of this character
	 * reference.
	 * <p>
	 * This is equivalent to {@link #getHexadecimalCharacterReferenceString(int) getHexadecimalCharacterReferenceString}<code>(</code>
	 * {@link #getCodePoint()}<code>)</code>.
	 * <p>
	 * <dl>
	 * <dt>Example:</dt>
	...
	/**
	 * Returns the <a href="NumericCharacterReference.html#HexadecimalCharacterReference">hexadecimal encoded form</a> of the specified
	 * unicode code point.
	 * <p>
	 * <dl>
	 * <dt>Example:</dt>
	...
	/**
	 * Returns the unicode code point of this character reference in <a target="_blank"
	 * href="http://www.unicode.org/reports/tr27/#notation">U+ notation</a>.
	 * <p>
	 ...
	 * <p>
	 * The character reference must be at the start of the given text, but may contain other characters at the end. The {@link #getEnd()
	 * getEnd()} method can be used on the resulting object to determine at which character position the character reference ended.
	 * <p>
	 ...
	 * </dl>
	 *
	 * @param characterReferenceText
	 *            the text containing a single encoded character reference.
	 * @return a <code>CharacterReference</code> object representing the specified text, or <code>null</code> if the text does not represent
	 ...
	 * <p>
	 * This is equivalent to {@link #parse(CharSequence) parse(characterReferenceText)}<code>.</code>{@link #getCodePoint()}, except that it
	 * returns {@link #INVALID_CODE_POINT} if an invalid character reference is specified instead of throwing a
	 ...
	 * <p>
	 * This is the case if a {@linkplain CharacterEntityReference character entity reference} exists for the character, or the unicode code
	 * point is greater than U+007F.
	 * <p>
	 * The only exception to this is an {@linkplain CharacterEntityReference#_apos apostrophe} (U+0027), which only returns
	 * <code>true</code> if the static {@link Config#IsApostropheEncoded} property is currently set to <code>true</code>.
	 *
	 * @param ch
	...
	/**
	 * Returns a filter <code>Writer</code> that {@linkplain #encode(CharSequence) encodes} all text before passing it through to the
	 * specified <code>Writer</code>.
	 *
	 * @param writer
	 *            the destination for the encoded text
	 * @return a filter <code>Writer</code> that {@linkplain #encode(CharSequence) encodes} all text before passing it through to the
	 *         specified <code>Writer</code>.
	...
	static Appendable appendEncode(final Appendable appendable, CharSequence unencodedText, final boolean whiteSpaceFormatting) throws IOException {
		...
		for (int i = beginPos; i < endPos; i++) {
			...
			// note that the last character is never a nbsp, so that word wrapping won't result in a nbsp before the first character in a
			// line
