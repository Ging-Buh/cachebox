...
final class StreamedText implements CharSequence {
	...
	public void setMinRequiredBufferBegin(final int minRequiredBufferBegin) {
		if (minRequiredBufferBegin < bufferBegin)
	...
	private void prepareBufferRange(final int begin, final int end) {
		...
		if (lastRequiredPos > readerPos)
			readToPosition(lastRequiredPos);
		checkPos(begin);
		if (end > this.end)
		...
		try {
			if (pos >= bufferBegin + buffer.length) {
				if (pos >= minRequiredBufferBegin + buffer.length) {
					if (!expandableBuffer)
						throw new BufferOverflowException(); // unfortunately BufferOverflowException doesn't accept a message argument, otherwise it would include the message "StreamedText buffer too small to keep positions "+minRequiredBufferBegin+" and "+pos+" simultaneously"
