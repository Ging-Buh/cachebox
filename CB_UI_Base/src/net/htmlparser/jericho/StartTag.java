...
public final class StartTag extends Tag {
	...
	if (element == Element.NOT_CACHED) {
		...
		if (endTag.element != Element.NOT_CACHED) {
		    ...
		    if (source.logger.isErrorEnabled() && !element.equals(endTag.element))
			source.logger.error(source.getRowColumnVector(endTag.begin).appendTo(new StringBuilder(200).append("End tag ").append(endTag).append(" at ")).append(" terminates more than one element").toString());
		}
	...
	if (HTMLElements.getEndTagForbiddenElementNames().contains(name))
	    return true;
	if (HTMLElements.getElementNames().contains(name))
	    return false;
	...
	if (attributes == null || attributes.containsServerTagOutsideOfAttributeValue)
	    return toString();
	...
	if (startTagType == StartTagType.NORMAL && getElement().getEndTag() == null && !HTMLElements.getEndTagOptionalElementNames().contains(name))
	    sb.append(" /");
	...
	if (startTagType != StartTagType.NORMAL)
	    sb.append('(').append(startTagType.getDescription()).append(") ");
	return sb;
    }
	...
	if (nextEndTag != null) {
		...
		if (source.logger.isErrorEnabled())
		    source.logger.error(source.getRowColumnVector(begin).appendTo(new StringBuilder(200).append("StartTag at ")).append(" missing required end tag - invalid nested start tag encountered before end tag").toString());
	...
	if (source.logger.isErrorEnabled())
	    source.logger.error(source.getRowColumnVector(begin).appendTo(new StringBuilder(200).append("StartTag at ")).append(" missing required end tag").toString());
	return null;
    }
	...
	while (pos < source.end) {
	    ...
	    if (terminatingTagNameSet != null && terminatingTagNameSet.contains(tag.name))
		return new EndTag(source, tag.begin, tag.begin, EndTagType.NORMAL, name);
	    pos = tag.begin + 1;
	}
	...
	if (startDelimiter.charAt(StartTagType.START_DELIMITER_PREFIX.length()) == '/')
	...
	try {
		...
		if (startTag == null)
		    continue; // keep looking if it wasn't a start tag
		if (searchStartTagType != startTag.getStartTagType()) {
		    ...
		    if (searchStartTagType != StartTagType.NORMAL || isXMLTagName || !startTag.isUnregistered())
			continue;
		}
		if (startTag.getStartTagType().isNameAfterPrefixRequired() && startTag.getName().length() > searchName.length()) {
		    ...
		    if (lastSearchNameChar != ':' && isXMLNameChar(lastSearchNameChar))
			continue;
		}
		return startTag;
	    } while ((begin -= 2) >= 0);
	...
	while (searchPos < source.end) {
		...
		if (startTag.getAttributes() != null) {
			...
			if (value.equalsIgnoreCase(attributeValue)) {
				...
				return startTag;
			    if (source.logger.isErrorEnabled())
				source.logger.error(source.getRowColumnVector(searchPos).appendTo(new StringBuilder(200)).append(": StartTag with attribute ").append(attributeName).append("=\"").append(attributeValue).append("\" ignored during search because its case does not match search value \"").append(value).append('"').toString());
			}
		    }
		}
	    }
	...
	while (searchPos < source.end) {
		...
		if (startTag.getAttributes() != null) {
			...
			if (attributeValue != null && regexPattern.matcher(attributeValue).matches())
			    return startTag;
		    }
		}
	    }
