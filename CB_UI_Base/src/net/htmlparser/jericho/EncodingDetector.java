...
final class EncodingDetector {
	...
	private EncodingDetector(final InputStream inputStream, final String preliminaryEncoding, final String preliminaryEncodingSpecificationInfo, final String alternativePreliminaryEncoding) throws IOException {
		...
		if (alternativePreliminaryEncoding != null && !isEncodingSupported(alternativePreliminaryEncoding))
	...
	public Reader openReader() throws UnsupportedEncodingException {
		if (encoding == null)
			return new InputStreamReader(inputStream, ISO_8859_1); // encoding==null only if input stream is empty so use an arbitrary encoding.
	...
	private boolean detectDocumentSpecifiedEncoding() throws IOException {
		...
		if (isEncodingSupported(preliminaryEncoding)) {
			...
			if (alternativePreliminaryEncoding == null)
				throw new UnsupportedEncodingException(preliminaryEncoding + ": " + preliminaryEncodingSpecificationInfo);
		...
		if (documentSpecifiedEncoding == null) {
			...
			try {
				...
				if (logger.isWarnEnabled())
					logger.warn("Unsupported encoding " + documentSpecifiedEncoding + " specified in document, using preliminary encoding " + safePreliminaryEncoding + " instead");
			} catch (IllegalCharsetNameException ex) {
				documentSpecifiedEncodingInfoSuffix = "illegal encoding " + documentSpecifiedEncoding + " specified in document";
				if (logger.isWarnEnabled())
					logger.warn("Illegal encoding " + documentSpecifiedEncoding + " specified in document, using preliminary encoding " + safePreliminaryEncoding + " instead");
			}
		}
		...
		if (preliminaryEncoding != safePreliminaryEncoding)
			return setEncoding(safePreliminaryEncoding, "alternative encoding substituted for unsupported preliminary encoding " + preliminaryEncoding + ": " + preliminaryEncodingSpecificationInfo + ", " + documentSpecifiedEncodingInfoSuffix);
	...
	boolean isIncompatibleWithPreliminaryEncoding(String encoding) {
		...
		if (preliminaryEncodingCodeUnitSize == null || encoding == null)
			return false;
