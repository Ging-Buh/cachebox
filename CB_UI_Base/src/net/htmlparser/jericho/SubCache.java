...
final class SubCache {
	...
	public Tag getTagAt(final int pos, final boolean serverTagOnly) {
		...
		if (cache.getSourceLength() == 0)
			return null;
		if (pos < 0 || pos >= cache.getSourceLength())
			return null;
	...
	protected StringBuilder appendTo(final StringBuilder sb) {
		...
		for (int i = 0; i <= lastIndex(); i++)
			sb.append(array[i]).append(Config.NewLine);
		return sb;
	}
			...
			if (!nextCacheEntry.previousCached) {
				...
				if (tag != null) {
					if (!cache.source.useAllTypesCache)
						addTagAt(tag.begin, tag); // have to add tag manually if useAllTypesCache is false
					return tag;
				}
			}
			if (previousCacheEntry == bof)
				return null;
	...
	private void addPreviousTag(final int pos, final Tag tag) {
		...
		if (tagPos == pos)
			return; // the tag was found exactly on pos, so cache has already been fully updated
		// tagPos < pos
		int index = getIndexOfPos(pos);
		...
		while (true) {
			...
			if (stepCacheEntry.tag != null) {
				if (stepCacheEntry.tag.includeInSearch())
					throw new SourceCacheEntryMissingInternalError(tagType, tag, this);
				stepCacheEntry.previousCached = true;
		...
		if (stepCacheEntry.pos != tagPos)
			throw new FoundCacheEntryMissingInternalError(tagType, tag, this);
		stepCacheEntry.nextCached = true;
