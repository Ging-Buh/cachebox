...
final class TagTypeRegister {
	...
	private static final TagType[] DEFAULT_TAG_TYPES = { StartTagType.UNREGISTERED, StartTagType.NORMAL, StartTagType.COMMENT, StartTagType.MARKUP_DECLARATION, StartTagType.DOCTYPE_DECLARATION, StartTagType.CDATA_SECTION,
			StartTagType.XML_PROCESSING_INSTRUCTION, StartTagType.XML_DECLARATION, StartTagType.SERVER_COMMON, StartTagType.SERVER_COMMON_ESCAPED, StartTagType.SERVER_COMMON_COMMENT, EndTagType.UNREGISTERED, EndTagType.NORMAL };
	...
	private void addTagTypesToList(final List<TagType> list) {
		if (tagTypes != null)
			for (int i = tagTypes.length - 1; i >= 0; i--)
				list.add(tagTypes[i]);
		if (children != null)
			for (TagTypeRegister tagTypeRegister : children)
	...
	static final class ProspectiveTagTypeIterator implements Iterator<TagType> {
		...
		public ProspectiveTagTypeIterator(final Source source, final int pos) {
			...
			} // not avoiding this exception is expensive but only happens in the very rare circumstance that the end of file is encountered in the middle of a potential tag.
				// go back up until we reach a node that contains a list of tag types:
			while (cursor.tagTypes == null)
				if ((cursor = cursor.parent) == null)
					break;
		}
		...
		public TagType next() {
			...
			if ((++tagTypeIndex) == tagTypes.length) {
				...
				do {
					cursor = cursor.parent;
				} while (cursor != null && cursor.tagTypes == null);
			}
			return nextTagType;
		}
	...
	private TagTypeRegister getChild(final char ch) {
		...
		if (children.length == 1)
			return children[0].ch == ch ? children[0] : null;
	...
	private void removeTagType(final TagType tagType) {
		...
		for (int i = 0; i < indexOfTagType; i++)
			newTagTypes[i] = tagTypes[i];
		for (int i = indexOfTagType; i < newTagTypes.length; i++)
			newTagTypes[i] = tagTypes[i + 1];
		tagTypes = newTagTypes;
	}
}
