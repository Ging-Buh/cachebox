...
public class Segment implements Comparable<Segment>, CharSequence
{
	...
	/**
	 * Constructs a new <code>Segment</code> within the specified {@linkplain Source source} document with the specified begin and end
	 * character positions.
	 * 
	 * @param source
	 *            the {@link Source} document, must not be <code>null</code>.
	 * @param begin
	 *            the character position in the source where this segment {@linkplain #getBegin() begins}, inclusive.
	 * @param end
	 ...
	 * <p>
	 * Use the {@link Source#getRowColumnVector(int pos)} method to determine the row and column numbers corresponding to this character
	 * position.
	 *
	 ...
	 * <p>
	 * Returns <code>true</code> if and only if the specified object is also a <code>Segment</code>, and both segments have the same
	 * {@link Source}, and the same begin and end positions.
	 * 
	 * @param object
	 *            the object to be compared for equality with this <code>Segment</code>.
	 ...
	 * <p>
	 * This is the case if {@link #getBegin()}<code>&lt;=segment.</code>{@link #getBegin()}<code> &amp;&amp; </code>{@link #getEnd()}
	 * <code>&gt;=segment.</code>{@link #getEnd()}.
	 * <p>
	 ...
	 *
	 * @param pos
	 *            the position in the {@link Source} document.
	 * @return <code>true</code> if this segment encloses the specified character position in the source document, otherwise
	 *         <code>false</code>.
	 */
	 ...
	 * <p>
	 * The returned <code>String</code> is newly created with every call to this method, unless this segment is itself an instance of
	 * {@link Source}.
	 *
	...
	/**
	 * Returns an iterator over every {@linkplain Tag tag}, {@linkplain CharacterReference character reference} and <a
	 ...
	 * <dd>
	 * <p>
	 * The following code demonstrates the typical usage of this method to make an exact copy of this segment to <code>writer</code>
	 ...
	 * </dl>
	 * 
	 * @return an iterator over every {@linkplain Tag tag}, {@linkplain CharacterReference character reference} and <a
	 *         href="Source.html#PlainText">plain text</a> segment contained within this segment.
	 */
	 ...
	 * <p>
	 * The {@link Source#fullSequentialParse()} method should be called after construction of the {@link Source} object if this method is to
	 * be used on a large proportion of the source. It is called automatically if this method is called on the {@link Source} object itself.
	 * <p>
	...
	/**
	 * Returns a list of all {@link Tag} objects of the specified {@linkplain TagType type} that are {@linkplain #encloses(Segment)
	 ...
	 *
	 * @param tagType
	 *            the {@linkplain TagType type} of tags to get.
	 * @return a list of all {@link Tag} objects of the specified {@linkplain TagType type} that are {@linkplain #encloses(Segment)
	 *         enclosed} by this segment.
	...
	/**
	 * Returns a list of all {@link StartTag} objects of the specified {@linkplain StartTagType type} that are
	 ...
	 *
	 * @param startTagType
	 *            the {@linkplain StartTagType type} of tags to get.
	 * @return a list of all {@link StartTag} objects of the specified {@linkplain StartTagType type} that are
	 *         {@linkplain #encloses(Segment) enclosed} by this segment.
	 */
	...
	/**
	 * Returns a list of all {@linkplain StartTagType#NORMAL normal} {@link StartTag} objects with the specified
	 * {@linkplain StartTag#getName() name} that are {@linkplain #encloses(Segment) enclosed} by this segment.
	 * <p>
	 * See the {@link Tag} class documentation for more details about the behaviour of this method.
	 * <p>
	 * Specifying a <code>null</code> argument to the <code>name</code> parameter is equivalent to {@link #getAllStartTags()}, which may
	 * include non-{@linkplain StartTagType#NORMAL normal} start tags.
	 * <p>
	 ...
	 *
	 * @param name
	 *            the {@linkplain StartTag#getName() name} of the start tags to get.
	 * @return a list of all {@linkplain StartTagType#NORMAL normal} {@link StartTag} objects with the specified
	 *         {@linkplain StartTag#getName() name} that are {@linkplain #encloses(Segment) enclosed} by this segment.
	 */
	...
	/**
	 * Returns a list of all {@link StartTag} objects with the specified attribute name/value pair that are {@linkplain #encloses(Segment)
	 * enclosed} by this segment.
	 * <p>
	 * See the {@link Tag} class documentation for more details about the behaviour of this method.
	 *
	 ...
	 * @return a list of all {@link StartTag} objects with the specified attribute name/value pair that are {@linkplain #encloses(Segment)
	 *         enclosed} by this segment.
	...
	/**
	 * Returns a list of all {@link StartTag} objects with the specified attribute name and value pattern that are
	 * {@linkplain #encloses(Segment) enclosed} by this segment.
	 * <p>
	 ...
	 * @return a list of all {@link StartTag} objects with the specified attribute name and value pattern that are
	 *         {@linkplain #encloses(Segment) enclosed} by this segment.
	...
	/**
	 * Returns a list of all {@link StartTag} objects with the specified class that are {@linkplain #encloses(Segment) enclosed} by this
	 * segment.
	 * <p>
	 * This matches start tags with a <code>class</code> attribute that contains the specified class name, either as an exact match or where
	 ...
	 * @return a list of all {@link StartTag} objects with the specified class that are {@linkplain #encloses(Segment) enclosed} by this
	 *         segment.
	 */
	 ...
	 * <p>
	 * An element found at the start of this segment is included in the list. Note however that if this segment <i>is</i> an {@link Element}
	 * , the overriding {@link Element#getChildElements()} method is called instead, which only returns the children of the element.
	 * <p>
	...
	/**
	 * Returns a list of all {@link Element} objects with the specified name that are {@linkplain #encloses(Segment) enclosed} by this
	 * segment.
	 * <p>
	 * The elements returned correspond with the start tags returned in the {@link #getAllStartTags(String name)} method, except that
	 * elements which are not entirely enclosed by this segment are excluded.
	 * <p>
	 * Specifying a <code>null</code> argument to the <code>name</code> parameter is equivalent to {@link #getAllElements()}, which may
	 * include elements of non-{@linkplain StartTagType#NORMAL normal} tags.
	 * <p>
	 ...
	 *
	 * @param name
	 *            the {@linkplain Element#getName() name} of the elements to get.
	 * @return a list of all {@link Element} objects with the specified name that are {@linkplain #encloses(Segment) enclosed} by this
	 *         segment.
	 */
	...
	/**
	 * Returns a list of all {@link Element} objects with start tags of the specified {@linkplain StartTagType type} that are
	 * {@linkplain #encloses(Segment) enclosed} by this segment.
	 * <p>
	 * The elements returned correspond with the start tags returned in the {@link #getAllTags(TagType)} method, except that elements which
	 * are not entirely enclosed by this segment are excluded.
	 * <p>
	 * If this segment is itself an {@link Element} with the specified type, the result includes this element in the list.
	 *
	 * @param startTagType
	 *            the {@linkplain StartTagType type} of start tags to get, must not be <code>null</code>.
	 * @return a list of all {@link Element} objects with start tags of the specified {@linkplain StartTagType type} that are
	 *         {@linkplain #encloses(Segment) enclosed} by this segment.
	 */
	...
	/**
	 * Returns a list of all {@link Element} objects with the specified attribute name/value pair that are {@linkplain #encloses(Segment)
	 * enclosed} by this segment.
	 * <p>
	 * The elements returned correspond with the start tags returned in the
	 * {@link #getAllStartTags(String attributeName, String value, boolean valueCaseSensitive)} method, except that elements which are not
	 * entirely enclosed by this segment are excluded.
	 * <p>
	 ...
	 * @param valueCaseSensitive
	 *            specifies whether the attribute value matching is case sensitive.
	 * @return a list of all {@link Element} objects with the specified attribute name/value pair that are {@linkplain #encloses(Segment)
	 *         enclosed} by this segment.
	...
	/**
	 * Returns a list of all {@link Element} objects with the specified attribute name and value pattern that are
	 * {@linkplain #encloses(Segment) enclosed} by this segment.
	 * <p>
	 * The elements returned correspond with the start tags returned in the
	 * {@link #getAllStartTags(String attributeName, Pattern valueRegexPattern)} method, except that elements which are not entirely
	 * enclosed by this segment are excluded.
	 * <p>
	 * Specifying a <code>null</code> argument to the <code>valueRegexPattern</code> parameter performs the search on the attribute name
	 * only, without regard to the attribute value. This will also match an attribute that {@linkplain Attribute#hasValue() has no value} at
	 * all.
	 * <p>
	 * If this segment is itself an {@link Element} with the specified attribute name and value pattern, the result includes this element in
	 * the list.
	 *
	 * @param attributeName
	 *            the attribute name (case insensitive) to search for, must not be <code>null</code>.
	 * @param valueRegexPattern
	 *            the regular expression pattern that must match the attribute value, may be <code>null</code>.
	 * @return a list of all {@link Element} objects with the specified attribute name and value pattern that are
	 *         {@linkplain #encloses(Segment) enclosed} by this segment.
	...
	/**
	 * Returns a list of all {@link Element} objects with the specified class that are {@linkplain #encloses(Segment) enclosed} by this
	 * segment.
	 * <p>
	 * This matches elements with a <code>class</code> attribute that contains the specified class name, either as an exact match or where
	 * the specified class name is one of multiple class names separated by white space in the attribute value.
	 * <p>
	 * The elements returned correspond with the start tags returned in the {@link #getAllStartTagsByClass(String className)} method, except
	 * that elements which are not entirely enclosed by this segment are excluded.
	 * <p>
	 * If this segment is itself an {@link Element} with the specified class, the result includes this element in the list.
	 *
	 * @param className
	 *            the class name (case sensitive) to search for, must not be <code>null</code>.
	 * @return a list of all {@link Element} objects with the specified class that are {@linkplain #encloses(Segment) enclosed} by this
	 *         segment.
	 */
	...
	/**
	 * Returns a list of all {@linkplain Attribute attributes} {@linkplain #encloses(Segment) enclosed} by this segment that have <a
	 * target="_blank" href="http://en.wikipedia.org/wiki/URI">URI</a> values.
	 * <p>
	 * According to the <a target="_blank" href="http://www.w3.org/TR/html401/">HTML 4.01 specification</a>, the following attributes have
	 * URI values:
	 * <table class="bordered" cellspacing="0">
	 * <tr>
	 * <th>HTML element name
	 * <th>Attribute name
	 * <tr>
	 * <td>{@link HTMLElementName#A A}
	 * <td>href
	 * <tr>
	 * <td>{@link HTMLElementName#APPLET APPLET}
	 * <td>codebase
	 * <tr>
	 * <td>{@link HTMLElementName#APPLET APPLET}
	 * <td>archive
	 * <tr>
	 * <td>{@link HTMLElementName#AREA AREA}
	 * <td>href
	 * <tr>
	 * <td>{@link HTMLElementName#BASE BASE}
	 * <td>href
	 * <tr>
	 * <td>{@link HTMLElementName#BLOCKQUOTE BLOCKQUOTE}
	 * <td>cite
	 * <tr>
	 * <td>{@link HTMLElementName#BODY BODY}
	 * <td>background
	 * <tr>
	 * <td>{@link HTMLElementName#FORM FORM}
	 * <td>action
	 * <tr>
	 * <td>{@link HTMLElementName#FRAME FRAME}
	 * <td>longdesc
	 * <tr>
	 * <td>{@link HTMLElementName#FRAME FRAME}
	 * <td>src
	 * <tr>
	 * <td>{@link HTMLElementName#DEL DEL}
	 * <td>cite
	 * <tr>
	 * <td>{@link HTMLElementName#HEAD HEAD}
	 * <td>profile
	 * <tr>
	 * <td>{@link HTMLElementName#IFRAME IFRAME}
	 * <td>longdesc
	 * <tr>
	 * <td>{@link HTMLElementName#IFRAME IFRAME}
	 * <td>src
	 * <tr>
	 * <td>{@link HTMLElementName#IMG IMG}
	 * <td>longdesc
	 * <tr>
	 * <td>{@link HTMLElementName#IMG IMG}
	 * <td>src
	 ...
	 * <td>usemap
	 * <tr>
	 * <td>{@link HTMLElementName#INPUT INPUT}
	 * <td>src
	 * <tr>
	 * <td>{@link HTMLElementName#INPUT INPUT}
	 * <td>usemap
	 * <tr>
	 * <td>{@link HTMLElementName#INS INS}
	 * <td>cite
	 * <tr>
	 * <td>{@link HTMLElementName#LINK LINK}
	 * <td>href
	 * <tr>
	 * <td>{@link HTMLElementName#OBJECT OBJECT}
	 * <td>archive
	 * <tr>
	 * <td>{@link HTMLElementName#OBJECT OBJECT}
	 * <td>classid
	 ...
	 * <td>usemap
	 * <tr>
	 * <td>{@link HTMLElementName#Q Q}
	 * <td>cite
	 * <tr>
	 * <td>{@link HTMLElementName#SCRIPT SCRIPT}
	 * <td>src
	 * </table>
	 * <p>
	 ...
	 *
	 * @return a list of all {@linkplain Attribute attributes} {@linkplain #encloses(Segment) enclosed} by this segment that have <a
	 *         target="_blank" href="http://en.wikipedia.org/wiki/URI">URI</a> values.
	...
	/**
	 * Returns a list of all <a target="_blank" href="http://en.wikipedia.org/wiki/URI">URI</a> {@linkplain Segment segments} inside the CSS
	 * of {@link HTMLElementName#STYLE STYLE} elements and <code>style</code> attribute values {@linkplain #encloses(Segment) enclosed} by
	 * this segment.
	 * <p>
	 * If this segment does not contain any tags, the entire segment is assumed to be CSS.
	 * <p>
	 * The URI segments are found by searching the CSS for the functional notation "<code>url()</code>" as described in <a target="_blank"
	 * href="http://www.w3.org/TR/CSS2/syndata.html#uri">section 4.3.4 of the CSS2 specification</a>.
	 * <p>
	 * The segments are returned in order of appearance.
	 *
	 * @return a list of all <a target="_blank" href="http://en.wikipedia.org/wiki/URI">URI</a> {@linkplain Segment segments} inside
	 *         {@link HTMLElementName#STYLE STYLE} elements and <code>style</code> attribute values {@linkplain #encloses(Segment) enclosed}
	 *         by this segment.
	 ...
	 * <p>
	 * This is functionally equivalent to {@link #getAllStartTags()}<code>.iterator().next()</code>, but does not search beyond the first
	 * start tag and returns <code>null</code> if no such start tag exists.
	 *
	...
	/**
	 * Returns the first {@link StartTag} of the specified {@linkplain StartTagType type} {@linkplain #encloses(Segment) enclosed} by this
	 * segment.
	 * <p>
	 * This is functionally equivalent to {@link #getAllStartTags(StartTagType) getAllStartTags(startTagType)}
	 * <code>.iterator().next()</code>, but does not search beyond the first start tag and returns <code>null</code> if no such start tag
	 * exists.
	 *
	 * @param startTagType
	 *            the <code>StartTagType</code> to search for.
	 * @return the first {@link StartTag} of the specified {@linkplain StartTagType type} {@linkplain #encloses(Segment) enclosed} by this
	 ...
	 * <p>
	 * This is functionally equivalent to {@link #getAllStartTags(String) getAllStartTags(name)}<code>.iterator().next()</code>, but does
	 * not search beyond the first start tag and returns <code>null</code> if no such start tag exists.
	 * <p>
	 ...
	 * @return the first {@linkplain StartTagType#NORMAL normal} {@link StartTag} {@linkplain #encloses(Segment) enclosed} by this segment,
	...
	/**
	 * Returns the first {@link StartTag} with the specified attribute name/value pair {@linkplain #encloses(Segment) enclosed} by this
	 * segment.
	 * <p>
	 * This is functionally equivalent to {@link #getAllStartTags(String,String,boolean)
	 * getAllStartTags(attributeName,value,valueCaseSensitive)}<code>.iterator().next()</code>, but does not search beyond the first start
	 * tag and returns <code>null</code> if no such start tag exists.
	 *
	 * @param attributeName
	 *            the attribute name (case insensitive) to search for, must not be <code>null</code>.
	 * @param value
	 ...
	 * @param valueCaseSensitive
	 *            specifies whether the attribute value matching is case sensitive.
	 * @return the first {@link StartTag} with the specified attribute name/value pair {@linkplain #encloses(Segment) enclosed} by this
	 *         segment, or <code>null</code> if none exists.
	...
	/**
	 * Returns the first {@link StartTag} with the specified attribute name and value pattern that is {@linkplain #encloses(Segment)
	 * enclosed} by this segment.
	 * <p>
	 ...
	 * @param valueRegexPattern
	 *            the regular expression pattern that must match the attribute value, may be <code>null</code>.
	 * @return the first {@link StartTag} with the specified attribute name and value pattern that is {@linkplain #encloses(Segment)
	 *         enclosed} by this segment, or <code>null</code> if none exists.
	 ...
	 * @param className
	 *            the class name (case sensitive) to search for, must not be <code>null</code>.
	 * @return the first {@link StartTag} with the specified class that is {@linkplain #encloses(Segment) enclosed} by this segment, or
	 ...
	 * <p>
	 * This is functionally equivalent to {@link #getAllElements()}<code>.iterator().next()</code>, but does not search beyond the first
	 * enclosed element and returns <code>null</code> if no such element exists.
	 * <p>
	...
	/**
	 * Returns the first {@linkplain StartTagType#NORMAL normal} {@link Element} with the specified {@linkplain Element#getName() name}
	 * {@linkplain #encloses(Segment) enclosed} by this segment.
	 * <p>
	 * This is functionally equivalent to {@link #getAllElements(String) getAllElements(name)}<code>.iterator().next()</code>, but does not
	 * search beyond the first enclosed element and returns <code>null</code> if no such element exists.
	 * <p>
	 ...
	 * @return the first {@linkplain StartTagType#NORMAL normal} {@link Element} with the specified {@linkplain Element#getName() name}
	...
	/**
	 * Returns the first {@link Element} with the specified attribute name/value pair {@linkplain #encloses(Segment) enclosed} by this
	 * segment.
	 * <p>
	 * This is functionally equivalent to {@link #getAllElements(String,String,boolean)
	 * getAllElements(attributeName,value,valueCaseSensitive)}<code>.iterator().next()</code>, but does not search beyond the first enclosed
	 * element and returns <code>null</code> if no such element exists.
	 * <p>
	 ...
	 * @param valueCaseSensitive
	 *            specifies whether the attribute value matching is case sensitive.
	 * @return the first {@link Element} with the specified attribute name/value pair {@linkplain #encloses(Segment) enclosed} by this
	 *         segment, or <code>null</code> if none exists.
	...
	/**
	 * Returns the first {@link Element} with the specified attribute name and value pattern that is {@linkplain #encloses(Segment)
	 * enclosed} by this segment.
	 * <p>
	 * This is functionally equivalent to {@link #getAllElements(String,Pattern) getAllElements(attributeName,valueRegexPattern)}
	 * <code>.iterator().next()</code>, but does not search beyond the first enclosed element and returns <code>null</code> if no such
	 * element exists.
	 * <p>
	 ...
	 * @param valueRegexPattern
	 *            the regular expression pattern that must match the attribute value, may be <code>null</code>.
	 * @return the first {@link Element} with the specified attribute name and value pattern that is {@linkplain #encloses(Segment)
	 *         enclosed} by this segment, or <code>null</code> if none exists.
	 ...
	 * @param className
	 *            the class name (case sensitive) to search for, must not be <code>null</code>.
	 * @return the first {@link Element} with the specified class that is {@linkplain #encloses(Segment) enclosed} by this segment, or
	 *         <code>null</code> if none exists.
	 */
	...
	/**
	 * Parses any {@link Attributes} within this segment. This method is only used in the unusual situation where attributes exist outside
	 * of a start tag. The {@link StartTag#getAttributes()} method should be used in normal situations.
	 * <p>
	 * This is equivalent to <code>source.</code>{@link Source#parseAttributes(int,int) parseAttributes}<code>(</code>{@link #getBegin()}
	 * <code>,</code>{@link #getEnd()}<code>)</code>.
	 *
	 ...
	 * {@linkplain StartTagType#NORMAL normal} tags from interfering with the parsing of the tags (such as where an {@linkplain Attribute
	 * attribute} of a normal tag uses a server tag to dynamically set its value), as well as preventing non-server tags from being
	 * recognised inside server tags.
	 * <p>
	 * It is not necessary to use this method to ignore {@linkplain TagType#isServerTag() server tags} located inside normal tags, as the
	 * attributes parser automatically ignores any server tags.
	 * <p>
	 * It is not necessary to use this method to ignore non-server tags inside server tags, or the contents of
	 * {@link HTMLElementName#SCRIPT SCRIPT} elements, as the parser does this automatically when performing a
	 ...
	 * <p>
	 * One such case is where XML-style server tags are used inside {@linkplain StartTagType#NORMAL normal} tags. Here is an example using
	 * an XML-style JSP tag: <blockquote class="code">
	 * <code>&lt;a href="&lt;i18n:resource path="/Portal"/&gt;?BACK=TRUE"&gt;back&lt;/a&gt;</code></blockquote> The first double-quote of
	 * <code>"/Portal"</code> will be interpreted as the end quote for the <code>href</code> attribute, as there is no way for the parser to
	 * recognise the <code>il8n:resource</code> element as a server tag. Such use of XML-style server tags inside
	 * {@linkplain StartTagType#NORMAL normal} tags is generally seen as bad practice, but it is nevertheless valid JSP. The only way to
	 * ensure that this library is able to parse the normal tag surrounding it is to find these server tags first and call the
	 * <code>ignoreWhenParsing</code> method to ignore them before parsing the rest of the document.
	 * <p>
	 * It is important to understand the difference between ignoring the segment when parsing and removing the segment completely. Any text
	 * inside a segment that is ignored when parsing is treated by most functions as content, and as such is included in the output of tools
	 * such as {@link TextExtractor} and {@link Renderer}.
	 * <p>
	 * To remove segments completely, create an {@link OutputDocument} and call its {@link OutputDocument#remove(Segment) remove(Segment)}
	 * or {@link OutputDocument#replaceWithSpaces(int,int) replaceWithSpaces(int begin, int end)} method for each segment. Then create a new
	 * source document using {@link Source#Source(CharSequence) new Source(outputDocument.toString())} and perform the desired operations on
	 * this new source object.
	 * <p>
	 ...
	 * {@linkplain Source#getCacheDebugInfo() tag cache}, and so will continue to be found by the <a href="Tag.html#TagSearchMethods">tag
	 * search methods</a>. If this is undesirable, the {@link Source#clearCache()} method can be called to remove them from the cache.
	 * Calling the {@link Source#fullSequentialParse()} method after this method clears the cache automatically.
	 * <p>
	 * For best performance, this method should be called on all segments that need to be ignored without calling any of the <a
	 * href="Tag.html#TagSearchMethods">tag search methods</a> in between.
	 *
	 ...
	 * <p>
	 * A segment is considered to be before another segment if its begin position is earlier, or in the case that both segments begin at the
	 * same position, its end position is earlier.
	 * <p>
	 * Segments that begin and end at the same position are considered equal for the purposes of this comparison, even if they relate to
	 * different source documents.
	 * <p>
	 * Note: this class has a natural ordering that is inconsistent with equals. This means that this method may return zero in some cases
	 * where calling the {@link #equals(Object)} method with the same argument returns <code>false</code>.
	 *
	 * @param segment
	 ...
	 *             if the argument is not a <code>Segment</code>
	 */
	@Override
	 ...
	 * 
	 * @return <code>true</code> if this segment consists entirely of {@linkplain #isWhiteSpace(char) white space}, otherwise
	 *         <code>false</code>.
	 */
	 ...
	 * <p>
	 * A high return value can indicate that the segment contains a large number of incorrectly nested tags that could result in a
	 * <code>StackOverflowException</code> if its content is parsed.
	 * <p>
	 ...
	 * The use of this method to pre-detect and avoid a stack overflow may save some memory and processing resources in certain
	 * circumstances, but the cost of calling this method to check every segment or document will very often exceed any benefit.
	 * <p>
	 * It is up to the application developer to determine what return value constitutes an unreasonable level of nesting given the stack
	 * space allocated to the application and other factors.
	 * <p>
	 * Note that the return value is an approximation only and is usually greater than the actual maximum element depth that would be
	 * reported by calling the {@link Element#getDepth()} method on the most nested element.
	 *
	...
	/**
	 * Indicates whether the specified character is <a target="_blank" href="http://www.w3.org/TR/html401/struct/text.html#h-9.1">white
	 * space</a>.
	 * <p>
	 * The <a target="_blank" href="http://www.w3.org/TR/html401/struct/text.html#h-9.1">HTML 4.01 specification section 9.1</a> specifies
	 * the following white space characters:
	 * <ul>
	 * <li>space (U+0020)
	 * <li>tab (U+0009)
	 * <li>form feed (U+000C)
	 ...
	 * </ul>
	 * <p>
	 * Despite the explicit inclusion of the zero-width space in the HTML specification, Microsoft IE6 does not recognise them as white
	 * space and renders them as an unprintable character (empty square). Even zero-width spaces included using the numeric character
	 * reference <code>&amp;#x200B;</code> are rendered this way.
	 *
	 * @param ch
	 *            the character to test.
	 * @return <code>true</code> if the specified character is <a target="_blank"
	...
	public static final boolean isWhiteSpace(final char ch)
	{
		for (char whiteSpaceChar : WHITESPACE)
			if (ch == whiteSpaceChar) return true;
		return false;
	}

	/**
	 * Returns a {@link RowColumnVector} object representing the row and column number of the {@linkplain #getBegin() start} of this segment
	 * in the source document.
	 * 
	 * @return a {@link RowColumnVector} object representing the row and column number of the {@linkplain #getBegin() start} of this segment
	 *         in the source document.
	 ...
	 * <p>
	 * This is logically equivalent to <code>toString().charAt(index)</code> for valid argument values <code>0 <= index < length()</code>.
	 * <p>
	 ...
	 * <code>IndexOutOfBoundsException</code> is thrown for an invalid argument value.
	 *
	 * @param index
	 ...
	 * <p>
	 * This is logically equivalent to <code>toString().subSequence(beginIndex,endIndex)</code> for valid values of <code>beginIndex</code>
	 * and <code>endIndex</code>.
	 * <p>
	 ...
	 * <code>IndexOutOfBoundsException</code> is thrown for invalid argument values as described in the
	 * <code>String.subSequence(int,int)</code> method.
	 *
	 * @param beginIndex
	...
	/**
	 * Collapses the {@linkplain #isWhiteSpace(char) white space} in the specified text. All leading and trailing white space is omitted,
