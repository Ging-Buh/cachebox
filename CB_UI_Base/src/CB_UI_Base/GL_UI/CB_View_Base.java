...
public abstract class CB_View_Base extends GL_View_Base implements ViewOptionsMenu {
	...
	return getName() + " X,Y/Width,Height = " + this.getX() + "," + this.getY() + "/" + this.getWidth() + ","
		+ this.getHeight();
    }
	...
	if (lastInRow) {
		...
		if (view.Weight > 0) {
			...
			fixedWidthSum += view.getWidth() + this.xMargin; // xMargin
									 // is
									 // added
									 // to
									 // each
	    ...
	    float objectWidth = 0;
	    if (weightedAnz != 0) {
		objectWidth = (widthToFill - percentWidthSum - fixedWidthSum + this.xMargin) / weightedAnz
			- this.xMargin;
	    }
	    for (int i = 0, n = this.row.size(); i < n; i++) {
		GL_View_Base view = this.row.get(i);
