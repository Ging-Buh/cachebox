...
public class Utils
{
	public static List<HtmlSegment> getAllSegments(String html, List<HtmlSegment> list, List<StartTag> aditionalTags)
	{
		HtmlSegment segment = getDeepestSegment(html);

		if (segment == null) return list;

		// is segment a inner segment?
		String beforeHtml = html.substring(0, segment.begin);
		HtmlSegment beforesegment = getDeepestSegment(beforeHtml);
	...
	public static HtmlSegment getDeepestSegment(String html)
	{
		...
		HtmlSegment ret = new HtmlSegment();
		ret.begin = start;
		ret.end = end.getEnd();
