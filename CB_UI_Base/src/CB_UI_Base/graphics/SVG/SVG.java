 ...
 * <h4>Usage summary</h4>
 * <ul>
 * <li>Use one of the static {@code getFromX()} methods to read and parse the SVG file. They will return an instance of this class.
...
public class SVG
{
	...
	public static ext_Bitmap createBmpFromSVG(ext_GraphicFactory factory, InputStream stream, float scale) throws SVGParseException
	{
		...
		if (scale != 1f)
		{
			bmcanvas.scale(scale, scale);
		}

		// Render our document onto our canvas
		svg.renderToCanvas(bmcanvas, viewPort, factory);
		return newBM;
	}
	...
	/**
	 * Set the DPI (dots-per-inch) value to use when rendering. The DPI setting is used in the conversion of "physical" units - such an "pt"
	 * or "cm" - to pixel values. The default DPI is 96.
	 * <p>
	...
	/**
	 * Renders this SVG document to a Canvas object. The full width and height of the canvas will be used as the viewport into which the
	...
	public void renderToCanvas(ext_Canvas canvas, RectF viewPort, ext_GraphicFactory factory)
	{
		...
		{
			svgViewPort = new Box(0f, 0f, canvas.getWidth(), canvas.getHeight());
		}

		SVGAndroidRenderer renderer = new SVGAndroidRenderer(canvas, svgViewPort, this.renderDPI, factory);
	 ...
	 * <p>
	 * A View is an special element in a SVG documents that describes a rectangular area in the document. Calling this method with a
	 * {@code viewId} will result in the specified view being positioned and scaled to the viewport. In other words, use
	 * {@link #renderToPicture()} to render the whole document, or use this method instead to render just a part of it.
	 * <p>
	...
	public void renderViewToCanvas(String viewId, ext_Canvas canvas, RectF viewPort, ext_GraphicFactory factory)
	{
		SvgObject obj = this.getElementById(viewId);
		if (obj == null) return;
		if (!(obj instanceof SVG.View)) return;
	...
	{
		if (this.rootElement == null) throw new IllegalArgumentException("SVG document is empty");
	 ...
	 * <p>
	 * If the width in the document is specified in pixels, that value will be returned. If the value is listed with a physical unit such as
	 * "cm", then the current {@code RenderDPI} value will be used to convert that value to pixels. If the width is missing, or in a form
	 ...
	 * <p>
	 * If the height in the document is specified in pixels, that value will be returned. If the value is listed with a physical unit such
	 * as "cm", then the current {@code RenderDPI} value will be used to convert that value to pixels. If the height is missing, or in a
	 * form which can't be converted to pixels, such as "100%" for example, -1 will be returned.
	 * 
	 ...
	 * <p>
	 * The viewBox generally describes the bounding box dimensions of the document contents. A valid viewBox is necessary if you want the
	 * document scaled to fit the canvas or viewport the document is to be rendered into.
	 * <p>
	...
	public RectF getDocumentViewBox()
	{
		...
		if (this.rootElement.viewBox == null) return null;

		return this.rootElement.viewBox.toRectF();
	}

	/**
	 * Change the document positioning by altering the "preserveAspectRatio" attribute of the root {@code <svg>} element. See the
	 * documentation for {@link PreserveAspectRatio} for more information on how positioning works.
	 * 
	...
	/**
	 * Return the "preserveAspectRatio" attribute of the root {@code <svg>} element in the form of an {@link PreserveAspectRatio} object.
	 * 
	...
	public PreserveAspectRatio getDocumentPreserveAspectRatio()
	{
		...
		if (this.rootElement.preserveAspectRatio == null) return null;
	 ...
	 * <p>
	 * If the width or height of the document are listed with a physical unit such as "cm", then the current {@code renderDPI} setting will
	 * be used to convert that value to pixels.
	 * <p>
	...
	private Box getDocumentDimensions(float dpi)
	{
		...
		if (w == null || w.isZero() || w.unit == Unit.percent || w.unit == Unit.em || w.unit == Unit.ex) return new Box(-1, -1, -1, -1);
	...
	protected static class Length implements Cloneable
	{
		...
		public float floatValueX(SVGAndroidRenderer renderer)
		{
			...
			case percent:
				...
				if (viewPortUser == null) return value; // Undefined in this situation - so just return value to avoid an NPE
	...
	protected static class TextContainer extends SvgConditionalContainer
	{
		...
		public void addChild(SvgObject elem) throws SAXException
		{
			if (elem instanceof TextChild) children.add(elem);
			else
	...
	protected static class GradientElement extends SvgElementBase implements SvgContainer
	{
		...
		public void addChild(SvgObject elem) throws SAXException
		{
			if (elem instanceof Stop) children.add(elem);
			else
	...
	{
		if (id.equals(rootElement.id)) return rootElement;
	...
	private SvgElementBase getElementById(SvgContainer obj, String id)
	{
		...
		if (id.equals(elem.id)) return elem;
		for (SvgObject child : obj.getChildren())
		{
			...
			if (id.equals(childElem.id)) return childElem;
	...
	private List<SvgObject> getElementsByTagName(SvgContainer obj, Class clazz)
	{
		...
		if (obj.getClass() == clazz) result.add((SvgObject) obj);
		for (SvgObject child : obj.getChildren())
		{
			if (child.getClass() == clazz) result.add(child);
			if (child instanceof SvgContainer) getElementsByTagName((SvgContainer) child, clazz);
		}
		return result;
	}
