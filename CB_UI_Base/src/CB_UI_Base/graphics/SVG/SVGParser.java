...
import CB_UI_Base.graphics.SVG.CSSParser.MediaType;
import CB_UI_Base.graphics.SVG.SVG.Box;
import CB_UI_Base.graphics.SVG.SVG.CSSClipRect;
...
import CB_UI_Base.graphics.SVG.SVG.Style.TextDecoration;
import CB_UI_Base.graphics.SVG.SVG.Style.TextDirection;
...
import CB_UI_Base.graphics.SVG.SVG.TextChild;
...
public class SVGParser extends DefaultHandler2
{
	...
	private enum SVGAttr
	{
		...
		{
			if (str.equals("class")) return CLASS;
			if (str.indexOf('_') != -1) return UNSUPPORTED;
			try
			{
	...
	public void characters(char[] ch, int start, int length) throws SAXException
	{
		...
		{
			if (metadataElementContents == null) metadataElementContents = new StringBuilder(length);
	...
	public void endElement(String uri, String localName, String qName) throws SAXException
	{
		...
		if (localName.equals(TAG_TITLE) || localName.equals(TAG_DESC))
		{
			...
			if (metadataTag.equals(TAG_TITLE)) svgDocument.setTitle(metadataElementContents.toString());
			else if (metadataTag.equals(TAG_DESC)) svgDocument.setDesc(metadataElementContents.toString());
	...
	public void endDocument() throws SAXException
	{
		...
		// Dump document
		if (LibConfig.DEBUG) dumpNode(svgDocument.getRootElement(), "");
	}
	...
	private void parseAttributesSVG(SVG.Svg obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case width:
				...
				if (obj.width.isNegative()) throw new SAXException("Invalid <svg> element. width cannot be negative");
				break;
			case height:
				obj.height = parseLength(val);
				if (obj.height.isNegative()) throw new SAXException("Invalid <svg> element. height cannot be negative");
				break;
			case version:
				obj.version = val;
				break;
	...
	private void parseAttributesUse(SVG.Use obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case width:
				obj.width = parseLength(val);
				if (obj.width.isNegative()) throw new SAXException("Invalid <use> element. width cannot be negative");
				break;
			case height:
				obj.height = parseLength(val);
				if (obj.height.isNegative()) throw new SAXException("Invalid <use> element. height cannot be negative");
				break;
			case href:
	...
	private void parseAttributesPath(SVG.Path obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case pathLength:
				...
				if (obj.pathLength < 0f) throw new SAXException("Invalid <path> element. pathLength cannot be negative");
				break;
			default:
				break;
			}
	...
	private void parseAttributesRect(SVG.Rect obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case width:
				obj.width = parseLength(val);
				if (obj.width.isNegative()) throw new SAXException("Invalid <rect> element. width cannot be negative");
				break;
			case height:
				obj.height = parseLength(val);
				if (obj.height.isNegative()) throw new SAXException("Invalid <rect> element. height cannot be negative");
				break;
			case rx:
				obj.rx = parseLength(val);
				if (obj.rx.isNegative()) throw new SAXException("Invalid <rect> element. rx cannot be negative");
				break;
			case ry:
				obj.ry = parseLength(val);
				if (obj.ry.isNegative()) throw new SAXException("Invalid <rect> element. ry cannot be negative");
				break;
			default:
				break;
			}
	...
	private void parseAttributesCircle(SVG.Circle obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case r:
				...
				if (obj.r.isNegative()) throw new SAXException("Invalid <circle> element. r cannot be negative");
				break;
			default:
				break;
			}
	...
	private void parseAttributesEllipse(SVG.Ellipse obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case rx:
				obj.rx = parseLength(val);
				if (obj.rx.isNegative()) throw new SAXException("Invalid <ellipse> element. rx cannot be negative");
				break;
			case ry:
				obj.ry = parseLength(val);
				if (obj.ry.isNegative()) throw new SAXException("Invalid <ellipse> element. ry cannot be negative");
				break;
			default:
				break;
			}
	...
	private void parseAttributesMarker(SVG.Marker obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case markerWidth:
				...
				if (obj.markerWidth.isNegative()) throw new SAXException("Invalid <marker> element. markerWidth cannot be negative");
				break;
			case markerHeight:
				obj.markerHeight = parseLength(val);
				if (obj.markerHeight.isNegative()) throw new SAXException("Invalid <marker> element. markerHeight cannot be negative");
				break;
			case markerUnits:
	...
	private void parseAttributesRadialGradient(SVG.SvgRadialGradient obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case r:
				obj.r = parseLength(val);
				if (obj.r.isNegative()) throw new SAXException("Invalid <radialGradient> element. r cannot be negative");
				break;
			case fx:
	...
	{
		if (val.length() == 0) throw new SAXException("Invalid offset value in <stop> (empty string)");
		int end = val.length();
	...
	private void parseAttributesPattern(SVG.Pattern obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case width:
				obj.width = parseLength(val);
				if (obj.width.isNegative()) throw new SAXException("Invalid <pattern> element. width cannot be negative");
				break;
			case height:
				obj.height = parseLength(val);
				if (obj.height.isNegative()) throw new SAXException("Invalid <pattern> element. height cannot be negative");
				break;
			case href:
				if (!XLINK_NAMESPACE.equals(attributes.getURI(i))) break;
				obj.href = val;
	...
	private void parseAttributesMask(SVG.Mask obj, Attributes attributes) throws SAXException
	{
		for (int i = 0; i < attributes.getLength(); i++)
		{
			...
			case width:
				obj.width = parseLength(val);
				if (obj.width.isNegative()) throw new SAXException("Invalid <mask> element. width cannot be negative");
				break;
			case height:
				obj.height = parseLength(val);
				if (obj.height.isNegative()) throw new SAXException("Invalid <mask> element. height cannot be negative");
				break;
			default:
				break;
			}
	...
	protected static class TextScanner
	{
		...
		public void skipWhitespace()
		{
			...
			{
				if (!isWhitespace(input.charAt(position))) break;
				position++;
			}
		}
		...
		public boolean skipCommaWhitespace()
		{
			...
			if (!(input.charAt(position) == ',')) return false;
			position++;
			skipWhitespace();
			return true;
		}
		...
		public Float nextFloat()
		{
			...
			if (floatEnd == position) return null;
		...
		/*
		 * Scans for a comma-whitespace sequence with a float following it. If found, the float is returned. Otherwise null is returned and
		...
		public Integer nextInteger()
		{
			...
			if (intEnd == position) return null;
		...
		public Length nextLength()
		{
			...
			Unit unit = nextUnit();
			if (unit == null) return new Length(scalar, Unit.px);
			else
		...
		protected int advanceChar()
		{
			...
			position++;
			if (position < input.length()) return input.charAt(position);
			else
				return -1;
		}

		/*
		 * Scans the input starting immediately at 'position' for the next token. A token is a sequence of characters terminating at a
		 * whitespace character. Note that this routine only checks for whitespace characters. Use nextToken(char) if token might end with
		 * another character.
		 */
		...
		/*
		 * Scans the input starting immediately at 'position' for the next token. A token is a sequence of characters terminating at either
		 * a whitespace character or the supplied terminating character.
		 */
		public String nextToken(char terminator)
		{
			...
			if (isWhitespace(ch) || ch == terminator) return null;
		...
		/*
		 * Scans the input starting immediately at 'position' for the a sequence of letter characters terminated by an open bracket. The
		...
		/*
		 * Scans the input starting immediately at 'position' for a floating point number. If one is found, the end position of the float
		 * will be returned. If the returned value is the same as 'position' then no float was found.
		 */
		...
		/*
		 * Scans the input starting immediately at 'position' for an integer number. If one is found, the end position of the float will be
		 * returned. If the returned value is the same as 'position' then no number was found.
		 */
		...
		public Unit nextUnit()
		{
			...
			if (position > (input.length() - 2)) return null;
			try
			{
	...
	protected static void processStyleProperty(Style style, String localName, String val) throws SAXException
	{
		...
		case stroke_dasharray:
			if (NONE.equals(val)) style.strokeDashArray = null;
			else
	...
	private ext_Matrix parseTransformList(String val) throws SAXException
	{
		...
		while (!scan.empty())
		{
			...
			if (cmd == null) throw new SAXException("Bad transform function encountered in transform list: " + val);
			...
			if (cmd.equals("matrix"))
			{
				...
				if (f == null || !scan.consume(')')) throw new SAXException("Invalid transform list: " + val);
			...
			else if (cmd.equals("translate"))
			{
				...
				if (tx == null || !scan.consume(')')) throw new SAXException("Invalid transform list: " + val);

				if (ty == null) matrix.preTranslate(tx, 0f);
				else
			...
			else if (cmd.equals("scale"))
			{
				...
				if (sx == null || !scan.consume(')')) throw new SAXException("Invalid transform list: " + val);

				if (sy == null) matrix.preScale(sx, sx);
				else
			...
			else if (cmd.equals("rotate"))
			{
				...
				if (ang == null || !scan.consume(')')) throw new SAXException("Invalid transform list: " + val);
	...
	/*
	 * Parse an SVG 'Length' value (usually a coordinate). Spec says: length ::= number ("em" | "ex" | "px" | "in" | "cm" | "mm" | "pt" |
	 * "pc" | "%")?
	 */
	protected static Length parseLength(String val) throws SAXException
	{
		if (val.length() == 0) throw new SAXException("Invalid length value (empty string)");
		int end = val.length();
		Unit unit = Unit.px;
	...
	{
		if (val.length() == 0) throw new SAXException("Invalid length list (empty string)");
		...
		while (!scan.empty())
		{
			...
			if (scalar == null) throw new SAXException("Invalid length list value: " + scan.ahead());
	...
	{
		if (val.length() == 0) throw new SAXException("Invalid float value (empty string)");
		try
		{
    ...
    */
	...
	private static SvgPaint parsePaintSpecifier(String val, String attrName) throws SAXException
	{
		if (val.startsWith("url("))
		{
			...
			if (closeBracket == -1) throw new SAXException("Bad " + attrName + " attribute. Unterminated url() reference");
	...
	private static Colour parseColour(String val) throws SAXException
	{
		...
		if (val.toLowerCase(Locale.US).startsWith("rgb("))
		{
			...
			scan.skipWhitespace();
			if (!scan.consume(')')) throw new SAXException("Bad rgb() colour value: " + val);
	...
	private static void parseFont(Style style, String val) throws SAXException
	{
		...
		if ("|caption|icon|menu|message-box|small-caption|status-bar|".indexOf('|' + val + '|') != -1) return;
		...
		while (true)
		{
			item = scan.nextToken('/');
			scan.skipWhitespace();
			if (item == null) throw new SAXException("Invalid font style attribute: missing font size and family");
		...
		if (scan.consume('/'))
		{
			scan.skipWhitespace();
			item = scan.nextToken();
			if (item == null) throw new SAXException("Invalid font style attribute: missing line-height");
			parseLength(item);
			scan.skipWhitespace();
		}
	...
	private static List<String> parseFontFamily(String val) throws SAXException
	{
		...
		while (true)
		{
			...
			if (item == null) item = scan.nextToken(',');
			if (item == null) break;
			if (fonts == null) fonts = new ArrayList<String>();
			fonts.add(item);
			scan.skipCommaWhitespace();
			if (scan.empty()) break;
		}
	...
	{
		if ("none".equals(val)) return Style.TextDecoration.None;
		if ("underline".equals(val)) return Style.TextDecoration.Underline;
		if ("overline".equals(val)) return Style.TextDecoration.Overline;
		if ("line-through".equals(val)) return Style.TextDecoration.LineThrough;
		if ("blink".equals(val)) return Style.TextDecoration.Blink;
	...
	{
		if ("ltr".equals(val)) return Style.TextDirection.LTR;
		if ("rtl".equals(val)) return Style.TextDirection.RTL;
	...
	{
		if ("butt".equals(val)) return Style.LineCaps.Butt;
		if ("round".equals(val)) return Style.LineCaps.Round;
		if ("square".equals(val)) return Style.LineCaps.Square;
	...
	private static Length[] parseStrokeDashArray(String val) throws SAXException
	{
		...
		if (dash.isNegative()) throw new SAXException("Invalid stroke-dasharray. Dash segemnts cannot be negative: " + val);
	...
	{
		if ("start".equals(val)) return Style.TextAnchor.Start;
		if ("middle".equals(val)) return Style.TextAnchor.Middle;
		if ("end".equals(val)) return Style.TextAnchor.End;
	...
	{
		if ("visible".equals(val) || "auto".equals(val)) return Boolean.TRUE;
		if ("hidden".equals(val) || "scroll".equals(val)) return Boolean.FALSE;
	...
	private static CSSClipRect parseClip(String val) throws SAXException
	{
		...
		scan.skipWhitespace();
		if (!scan.consume(')')) throw new SAXException("Bad rect() clip definition: " + val);
	...
	{
		if (scan.consume("auto")) return new Length(0f);
	...
	{
		if ("none".equals(val)) return Style.VectorEffect.None;
		if ("non-scaling-stroke".equals(val)) return Style.VectorEffect.NonScalingStroke;
