...
import CB_UI_Base.graphics.SVG.SVG.Box;
import CB_UI_Base.graphics.SVG.SVG.ClipPath;
import CB_UI_Base.graphics.SVG.SVG.Colour;
import CB_UI_Base.graphics.SVG.SVG.CurrentColor;
import CB_UI_Base.graphics.SVG.SVG.GradientElement;
import CB_UI_Base.graphics.SVG.SVG.GradientSpread;
import CB_UI_Base.graphics.SVG.SVG.Length;
import CB_UI_Base.graphics.SVG.SVG.Line;
import CB_UI_Base.graphics.SVG.SVG.Marker;
import CB_UI_Base.graphics.SVG.SVG.NotDirectlyRendered;
import CB_UI_Base.graphics.SVG.SVG.PaintReference;
import CB_UI_Base.graphics.SVG.SVG.PathDefinition;
import CB_UI_Base.graphics.SVG.SVG.PathInterface;
import CB_UI_Base.graphics.SVG.SVG.Pattern;
import CB_UI_Base.graphics.SVG.SVG.Rect;
import CB_UI_Base.graphics.SVG.SVG.SolidColor;
import CB_UI_Base.graphics.SVG.SVG.Stop;
import CB_UI_Base.graphics.SVG.SVG.Style;
import CB_UI_Base.graphics.SVG.SVG.Style.TextAnchor;
import CB_UI_Base.graphics.SVG.SVG.Style.VectorEffect;
import CB_UI_Base.graphics.SVG.SVG.SvgContainer;
import CB_UI_Base.graphics.SVG.SVG.SvgElement;
import CB_UI_Base.graphics.SVG.SVG.SvgElementBase;
import CB_UI_Base.graphics.SVG.SVG.SvgLinearGradient;
import CB_UI_Base.graphics.SVG.SVG.SvgObject;
import CB_UI_Base.graphics.SVG.SVG.SvgPaint;
import CB_UI_Base.graphics.SVG.SVG.SvgRadialGradient;
...
public class SVGAndroidRenderer
{
	...
	private class RendererState implements Cloneable
	{
		...
		protected Object clone()
		{
			...
			try
			{
				...
				obj.fillPaint = GRAPHIC_FACTORY.createPaint(fillPaint);
				obj.strokePaint = GRAPHIC_FACTORY.createPaint(strokePaint);
				return obj;
			}
	...
	{
		if (state.viewBox != null) return state.viewBox;
		else
	...
	{
		if (obj instanceof NotDirectlyRendered) return;
	...
	private void updateStyleForElement(RendererState state, SvgElementBase obj)
	{
		...
		if (obj.baseStyle != null) updateStyle(state, obj.baseStyle);
	...
	{
		if (!(obj instanceof SvgElementBase)) return;

		SvgElementBase bobj = (SvgElementBase) obj;
		if (bobj.spacePreserve != null) state.spacePreserve = bobj.spacePreserve;
	}
	...
	private void doStroke(ext_Path path)
	{
		...
		if (state.style.vectorEffect == VectorEffect.NonScalingStroke)
		{
			...
			if (currentShaderMatrix != null) state.strokePaint.setGradientMatrix(currentShaderMatrix);
		}
		else
		{
	...
	private void render(SVG.Svg obj, SVG.Length width, SVG.Length height, Box viewBox, PreserveAspectRatio positioning)
	{
		...
		if (positioning == null) positioning = (obj.preserveAspectRatio != null) ? obj.preserveAspectRatio : PreserveAspectRatio.LETTERBOX;
	...
	private void updateParentBoundingBox(SvgElement obj)
	{
		...
		if (matrixStack.peek().invert())
		{
			...
			if (parent.boundingBox == null) parent.boundingBox = Box.fromLimits(rect.left, rect.top, rect.right, rect.bottom);
			else
	...
	{
		if (!requiresCompositing()) return false;
	...
	private void popLayer(SvgElement obj)
	{
		...
		if (state.style.mask != null && state.directRendering)
		{
			...
			canvas.drawBitmap(maskedContent, 0, 0);
	...
	{
		if (state.style.mask != null && !state.directRendering) warn("Masks are not supported when using getPicture()");
	...
	private void render(SVG.Use obj)
	{
		...
		if ((obj.width != null && obj.width.isZero()) || (obj.height != null && obj.height.isZero())) return;
	...
	private void render(SVG.Path obj)
	{
		...
		if (!display()) return;
		if (!visible()) return;
		if (!state.hasStroke && !state.hasFill) return;

		if (obj.transform != null) canvas.concat(obj.transform);
	...
	private void render(SVG.Rect obj)
	{
		...
		if (obj.width == null || obj.height == null || obj.width.isZero() || obj.height.isZero()) return;
	...
	private void render(SVG.Circle obj)
	{
		...
		if (obj.r == null || obj.r.isZero()) return;
	...
	private void render(SVG.Ellipse obj)
	{
		...
		if (obj.rx == null || obj.ry == null || obj.rx.isZero() || obj.ry.isZero()) return;
	...
	{
		if (state.style.direction == Style.TextDirection.LTR || state.style.textAnchor == TextAnchor.Middle) return state.style.textAnchor;
	...
	private class PlainTextDrawer extends TextProcessor
	{
		...
		public void processText(String text)
		{
			...
			if (visible())
			{
				if (state.hasFill) canvas.drawText(text, x, y, state.fillPaint);
				if (state.hasStroke) canvas.drawText(text, x, y, state.strokePaint);
			}
	...
	/*
	 * Given a text container, recursively visit its children invoking the TextDrawer handler for each segment of text found.
	 */
	...
	private void processTextChild(SVG.SvgObject obj, TextProcessor textprocessor)
	{
		...
		if (!textprocessor.doTextContainer((SVG.TextContainer) obj)) return;
	...
	private void renderTextPath(SVG.TextPath obj)
	{
		...
		if (pathObj.transform != null) path.transform(pathObj.transform);
	...
	private class PathTextDrawer extends PlainTextDrawer
	{
		...
		public void processText(String text)
		{
			if (visible())
			{
				if (state.hasFill) canvas.drawTextOnPath(text, path, x, y, state.fillPaint);
				if (state.hasStroke) canvas.drawTextOnPath(text, path, x, y, state.strokePaint);
			}
	...
	/*
	 * Calculate the approximate width of this line of text. To simplify, we will ignore font changes and just assume that all the text uses
	 * the current font.
	 */
	...
	private class TextBoundsCalculator extends TextProcessor
	{
		...
		public void processText(String text)
		{
			if (visible())
			{
				...
				float textWidth = state.fillPaint.getTextWidth(text);
				float textHeight = state.fillPaint.getTextHeight(text);
				RectF textbounds = new RectF(0, 0, textWidth, textHeight);
	...
	private String textXMLSpaceTransform(String text, boolean isFirstChild, boolean isLastChild)
	{
		...
		text = text.replaceAll("\\t", " ");
		// text = text.trim();
		if (isFirstChild) text = text.replaceAll("^\\s+", "");
		if (isLastChild) text = text.replaceAll("\\s+$", "");
	...
	/*
	 * Check for an decode an image encoded in a data URL. We don't handle all permutations of data URLs. Only base64 ones.
	 */
	private Bitmap checkForImageDataURL(String url)
	{
		if (!url.startsWith("data:")) return null;
		if (url.length() < 14) return null;

		int comma = url.indexOf(',');
		if (comma == -1 || comma < 12) return null;
		if (!";base64".equals(url.substring(comma - 7, comma))) return null;
		// TODO handle!
	...
	{
		if (state.style.display != null) return state.style.display;
		return true;
	}

	private boolean visible()
	{
		if (state.style.visibility != null) return state.style.visibility;
		return true;
	}

	/*
	 * Calculate the transform required to fit the supplied viewBox into the current viewPort. See spec section 7.8 for an explanation of
	 * how this works.
	 * 
	 * aspectRatioRule determines where the graphic is placed in the viewPort when aspect ration is kept. xMin means left justified, xMid is
	 * centred, xMax is right justified etc. slice determines whether we see the whole image or not. True fill the whole viewport. If slice
	 * is false, the image will be "letter-boxed".
	 * 
	...
	/*
	 * Updates the global style state with the style defined by the current object. Will also update the current paints etc where
	 * appropriate.
	 */
	private void updateStyle(RendererState state, Style style)
	{
		...
		if (isSpecified(style, SVG.SPECIFIED_STROKE_DASHARRAY | SVG.SPECIFIED_STROKE_DASHOFFSET))
		{
			...
			{
				state.strokePaint.delDashPathEffect();// setDashPathEffect(null);
			}
			else
			{
				...
				if (intervalSum == 0f)
				{
					state.strokePaint.delDashPathEffect();
				}
				else
				{
					...
					state.strokePaint.setDashPathEffect(intervals, offset);
				}
			}
		}
		...
		if (isSpecified(style, SVG.SPECIFIED_FONT_WEIGHT))
		{
			...
			if (style.fontWeight == Style.FONT_WEIGHT_LIGHTER && state.style.fontWeight > 100) state.style.fontWeight -= 100;
			else if (style.fontWeight == Style.FONT_WEIGHT_BOLDER && state.style.fontWeight < 900) state.style.fontWeight += 100;
			else
	...
	private void setPaintColour(RendererState state, boolean isFill, SvgPaint paint)
	{
		...
		paintOpacity *= state.style.opacity;
		int col;
		...
		col = GRAPHIC_FACTORY.setColorAlpha(col, paintOpacity);
		// col = clamp255(paintOpacity) << 24 | col;
		if (isFill)
		{
			state.fillPaint.setColor(col);
			// state.fillPaint.setAlpha((int) (paintOpacity*255));
	...
	{
		if (state.style.fillRule == null) return ext_Path.FillType.WINDING;
	...
	private void viewportFill()
	{
		...
		if (state.style.viewportFillOpacity != null) col = GRAPHIC_FACTORY.setColorAlpha(col, state.style.viewportFillOpacity);
	...
	{
		GL_Path path = (GL_Path) GRAPHIC_FACTORY.createPath();
		float lastX, lastY;
		...
		{
			SVGAndroidRenderer.arcTo(lastX, lastY, rx, ry, xAxisRotation, largeArcFlag, sweepFlag, x, y, this, GRAPHIC_FACTORY);
			lastX = x;
			lastY = y;
		}
		...
		public void close()
		{
			...
			// float lastX = path.items[path.size - 2];
			// float lastY = path.items[path.size - 1];
			// path.items[0] = lastX;
			// path.items[1] = lastY;
	...
	/*
	 * SVG arc representation uses "endpoint parameterisation" where we specify the endpoint of the arc. This is to be consistent with the
	 * other path commands. However we need to convert this to "centre point parameterisation" in order to calculate the arc. Handily, the
	 * SVG spec provides all the required maths in section "F.6 Elliptical arc implementation notes".
	 * 
	...
	/*
	 * Generate the control points and endpoints for a set of bezier curves that match a circular arc starting from angle 'angleStart' and
	 * sweep the angle 'angleExtent'. The circle the arc follows will be centred on (0,0) and have a radius of 1.0.
	 * 
	...
	{
		private final List<MarkerVector> markers = new ArrayList<MarkerVector>();
		...
		public void arcTo(float rx, float ry, float xAxisRotation, boolean largeArcFlag, boolean sweepFlag, float x, float y)
		{
			...
			startArc = true;
			normalCubic = false;
			SVGAndroidRenderer.arcTo(lastPos.x, lastPos.y, rx, ry, xAxisRotation, largeArcFlag, sweepFlag, x, y, this, GRAPHIC_FACTORY);
			normalCubic = true;
			closepathReAdjustPending = false;
		}
	...
	{
		if (state.style.markerStart == null && state.style.markerMid == null && state.style.markerEnd == null) return;
		...
		if (state.style.markerStart != null)
		{
			...
			if (ref != null) _markerStart = (SVG.Marker) ref;
			else
		...
		if (state.style.markerMid != null)
		{
			...
			if (ref != null) _markerMid = (SVG.Marker) ref;
			else
		...
		if (state.style.markerEnd != null)
		{
			...
			if (ref != null) _markerEnd = (SVG.Marker) ref;
			else
				error("Marker reference '%s' not found", state.style.markerEnd);
		}

		List<MarkerVector> markers = null;
		if (obj instanceof SVG.Path) markers = (new MarkerPositionCalculator(((SVG.Path) obj).d)).getMarkers();
		else if (obj instanceof SVG.Line) markers = calculateMarkerPositions((SVG.Line) obj);
		else
		...
		if (_markerStart != null) renderMarker(_markerStart, markers.get(0));
		...
		if (_markerEnd != null) renderMarker(_markerEnd, markers.get(markerCount - 1));
	}
	...
	private void renderMarker(Marker marker, MarkerVector pos)
	{
		...
		if (compositing) popLayer(marker);

		statePop();
	}

	/*
	 * Determine an elements style based on it's ancestors in the tree rather than it's render time ancestors.
	 */
	...
	/*
	 * Check for gradiant fills or strokes on this object. These are always relative to the object, so can't be preconfigured. They have to
	 * be initialised at the time each object is rendered.
	 */
	...
	private void decodePaintReference(boolean isFill, Box boundingBox, PaintReference paintref)
	{
		...
		if (ref == null)
		{
			...
			{
				if (isFill) state.hasFill = false;
				else
					state.hasStroke = false;
			}
			return;
		}
		if (ref instanceof SvgLinearGradient) makeLinearGradiant(isFill, boundingBox, (SvgLinearGradient) ref);
		if (ref instanceof SvgRadialGradient) makeRadialGradiant(isFill, boundingBox, (SvgRadialGradient) ref);
		if (ref instanceof SolidColor) setSolidColor(isFill, (SolidColor) ref);
	...
	{
		if (gradient.href != null) fillInChainedGradientFields(gradient, gradient.href);
		...
		if (gradient.gradientTransform != null)
		{
			...
			m.postConcat(gradient.gradientTransform);
		}
		...
		for (SvgObject child : gradient.children)
		{
			...
			if (col == null) col = Colour.BLACK;
			colours[i] = GRAPHIC_FACTORY.setColorAlpha(col.colour, state.style.stopOpacity);
		...
		{
			if (gradient.spreadMethod == GradientSpread.reflect) tileMode = TileMode.MIRROR;
			else if (gradient.spreadMethod == GradientSpread.repeat) tileMode = TileMode.REPEAT;
		}

		statePop();

		// Create shader instance
		paint.setLinearGradient(_x1, _y1, _x2, _y2, colours, positions, tileMode);
		paint.setGradientMatrix(m);
	}
	...
	private void makeRadialGradiant(boolean isFill, Box boundingBox, SvgRadialGradient gradient)
	{
		...
		// Create shader instance
		paint.setRadialGradiant(_cx, _cy, _r, colours, positions, tileMode);
		paint.setGradientMatrix(m);
	}
	...
	private void fillInChainedGradientFields(GradientElement gradient, String href)
	{
		...
		if (gradient.gradientUnitsAreUser == null) gradient.gradientUnitsAreUser = grRef.gradientUnitsAreUser;
		if (gradient.gradientTransform == null) gradient.gradientTransform = grRef.gradientTransform;
		if (gradient.spreadMethod == null) gradient.spreadMethod = grRef.spreadMethod;
		if (gradient.children.isEmpty()) gradient.children = grRef.children;
		...
		if (grRef.href != null) fillInChainedGradientFields(gradient, grRef.href);
	}
	...
	{
		if (state.style.clipPath == null) return;
	...
	{
		if (state.style.clipRule == null) return ext_Path.FillType.WINDING;
	...
	private void addObjectToClip(SVG.Path obj, ext_Path combinedPath, ext_Matrix combinedPathMatrix)
	{
		...
		// if (obj.transform != null) combinedPathMatrix.preConcat(obj.transform);
		if (obj.transform != null) combinedPathMatrix.postConcat(obj.transform);
	...
	private void addObjectToClip(SVG.GraphicsElement obj, ext_Path combinedPath, ext_Matrix combinedPathMatrix)
	{
		...
		ext_Path path;
		if (obj instanceof SVG.Rect) path = makePathAndBoundingBox((SVG.Rect) obj);
		else if (obj instanceof SVG.Circle) path = makePathAndBoundingBox((SVG.Circle) obj);
		else if (obj instanceof SVG.Ellipse) path = makePathAndBoundingBox((SVG.Ellipse) obj);
		else if (obj instanceof SVG.PolyLine) path = makePathAndBoundingBox((SVG.PolyLine) obj);
		else
			return;
	...
	private ext_Path makePathAndBoundingBox(SVG.PolyLine obj)
	{
		...
		if (obj instanceof SVG.Polygon) path.close();
	...
	private void fillWithPattern(SvgElement obj, ext_Path path, Pattern pattern)
	{
		...
		if (pattern.href != null) fillInChainedPatternFields(pattern, pattern.href);
		...
		if (pattern.patternTransform != null)
		{
			...
			// Matrix inverse = GRAPHIC_FACTORY.createMatrix();
		...
		for (float stepY = originY; stepY < bottom; stepY += h)
		{
			for (float stepX = originX; stepX < right; stepX += w)
			{
				...
				if (compositing) popLayer(pattern);
	...
	/*
	 * Any unspecified fields in this pattern can be 'borrowed' from another pattern specified by the href attribute.
	 */
	private void fillInChainedPatternFields(Pattern pattern, String href)
	{
		...
		if (pattern.patternUnitsAreUser == null) pattern.patternUnitsAreUser = pRef.patternUnitsAreUser;
		if (pattern.patternContentUnitsAreUser == null) pattern.patternContentUnitsAreUser = pRef.patternContentUnitsAreUser;
		if (pattern.patternTransform == null) pattern.patternTransform = pRef.patternTransform;
		if (pattern.x == null) pattern.x = pRef.x;
		if (pattern.y == null) pattern.y = pRef.y;
		if (pattern.width == null) pattern.width = pRef.width;
		if (pattern.height == null) pattern.height = pRef.height;
		// attributes from superclasses
		if (pattern.children.isEmpty()) pattern.children = pRef.children;
		if (pattern.viewBox == null) pattern.viewBox = pRef.viewBox;
		if (pattern.preserveAspectRatio == null)
		{
			pattern.preserveAspectRatio = pRef.preserveAspectRatio;
		}

		if (pRef.href != null) fillInChainedPatternFields(pattern, pRef.href);
	}
