...
public class Core implements ApplicationListener {
	...
	private ArrayList<CopyRule> getCopyRulesLibGdx() {
		...
		rules.add(new CopyRule(cs + "\\extensions\\gdx-freetype\\armeabi-v7a\\libgdx-freetype.so", workPath + "\\Android_GUI\\libs\\armeabi-v7a"));
		...
		rules.add(new CopyRule(cs + "\\extensions\\gdx-freetype\\gdx-freetype.jar", workPath + "\\refresh_libraries\\refresh_libraries\\libs"));
		rules.add(new CopyRule(cs + "\\extensions\\gdx-freetype\\gdx-freetype-natives.jar", workPath + "\\refresh_libraries\\refresh_libraries\\libs"));
		...
		rules.add(new CopyRule(cs + "\\extensions\\gdx-freetype\\gdx-freetype.jar", workPath + "\\refresh_libraries\\refresh_libraries-desktop\\libs"));
		rules.add(new CopyRule(cs + "\\extensions\\gdx-freetype\\gdx-freetype-natives.jar", workPath + "\\refresh_libraries\\refresh_libraries-desktop\\libs"));
	...
	ClickListener selectTrunkListener = new ClickListener() {
		@Override
	...
	ClickListener runListenerLibGdx = new ClickListener() {
		@Override
		public void clicked(InputEvent event, float x, float y) {
	...
	ClickListener runListenerTranslations = new ClickListener() {
		@Override
		public void clicked(InputEvent event, float x, float y) {
			Thread t = new Thread(new Runnable() {
	...
	ClickListener runListenerTexture = new ClickListener() {
		@Override
		public void clicked(InputEvent event, float x, float y) {
			Thread t = new Thread(new Runnable() {
	...
	private void layout() {
		...
		if (WorkPathFound > 0) {
			...
			if (ImageWorkPathFound)
				btnRunTexturePacker.setVisible(true);
			else
			...
			File[] files = dir.listFiles(new FileFilter() {
				...
				public boolean accept(File pathname) {
					if (pathname.getName().endsWith(".spp")) {
						if (pathname.getName().endsWith("_MipMap.spp"))
							return false;
						return true;
					}
					return false;
