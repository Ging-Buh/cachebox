...
public class SearchForGeocaches {
	...
	public static String SearchForGeocachesJSON(String accessToken,
			Search search, ArrayList<Cache> cacheList,
			ArrayList<LogEntry> logList, long gpxFilenameId) {
		String result = "";
			...
			try {
				...
				if (status.getInt("StatusCode") == 0) {
					...
					for (int i = 0; i < caches.length(); i++) {
						...
						cache.Size = CacheSizes.parseInt(GroundspeakAPI
								.getCacheSize(jSize));
						...
						cache.Type = GroundspeakAPI.getCacheType(jCacheType
						...
						for (int j = 0; j < logs.length(); j++) {
							...
							log.Type = GroundspeakAPI.getLogType(jLogType
									.getInt("WptLogTypeId"));
							logList.add(log);
						}
						...
						for (int j = 0; j < waypoints.length(); j++) {
							...
							waypoint.Type = GroundspeakAPI
									.getCacheType(jWaypoints
											.getInt("WptTypeID"));
